...
Data: Auto-downloaded at runtime via Kaggle (jessemostipak/hotel-booking-demand)
       Booking records are aggregated to daily non-cancelled arrivals.
"""
...
def load_data():
    import os, glob as _glob, calendar
    ...
    if not _csvs:
        ...
        _api.dataset_download_files("jessemostipak/hotel-booking-demand", path=_data_dir, unzip=True)
        _csvs = _glob.glob(os.path.join(_data_dir, "**", "*.csv"), recursive=True)
        print("Downloaded jessemostipak/hotel-booking-demand from Kaggle")
    _fp = sorted(_csvs, key=os.path.getsize, reverse=True)[0]
    df = pd.read_csv(_fp)
    # Filter non-cancelled bookings
    if "is_canceled" in df.columns:
        df = df[df["is_canceled"] == 0].copy()
    # Reconstruct arrival date from year/month/day columns
    month_map = {m: i for i, m in enumerate(calendar.month_name) if m}
    if "arrival_date_month" in df.columns:
        df["arrival_month_num"] = df["arrival_date_month"].map(month_map)
    else:
        df["arrival_month_num"] = 1
    year_col = [c for c in df.columns if "arrival" in c.lower() and "year" in c.lower()]
    day_col = [c for c in df.columns if "arrival" in c.lower() and "day" in c.lower()]
    if year_col and day_col:
        df["arrival_date"] = pd.to_datetime(
            df[year_col[0]].astype(str) + "-" +
            df["arrival_month_num"].astype(str) + "-" +
            df[day_col[0]].astype(str), errors="coerce")
    else:
        for c in df.columns:
            if "date" in c.lower(): df["arrival_date"] = pd.to_datetime(df[c], errors="coerce"); break
    df = df.dropna(subset=["arrival_date"])
    # Aggregate: count arrivals per day
    daily = df.groupby("arrival_date").size().reset_index(name="arrivals")
    daily = daily.sort_values("arrival_date").set_index("arrival_date")
    full_idx = pd.date_range(daily.index.min(), daily.index.max(), freq="D")
    daily = daily.reindex(full_idx, fill_value=0); daily.index.name = "arrival_date"
    target = TARGET
