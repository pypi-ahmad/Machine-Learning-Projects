...
Data: Auto-downloaded at runtime via Kaggle (arnabbiswas1/microsoft-azure-predictive-maintenance)
       Multi-machine hourly telemetry is aggregated to daily mean voltage.
"""
...
def load_data():
    ...
    _target_file = None
    _csvs = _glob.glob(os.path.join(_data_dir, "**", "PdM_telemetry.csv"), recursive=True)
    if _csvs:
        _target_file = _csvs[0]
    if _target_file is None:
        ...
        if not _all:
            ...
            _api.dataset_download_files("arnabbiswas1/microsoft-azure-predictive-maintenance", path=_data_dir, unzip=True)
            _csvs = _glob.glob(os.path.join(_data_dir, "**", "PdM_telemetry.csv"), recursive=True)
            if _csvs: _target_file = _csvs[0]
            print("Downloaded arnabbiswas1/microsoft-azure-predictive-maintenance from Kaggle")
        if _target_file is None:
            _all = _glob.glob(os.path.join(_data_dir, "**", "*.csv"), recursive=True)
            # Pick telemetry file by name or largest
            for f in _all:
                if "telemetry" in os.path.basename(f).lower(): _target_file = f; break
            if _target_file is None: _target_file = sorted(_all, key=os.path.getsize, reverse=True)[0]
    df = pd.read_csv(_target_file, nrows=500000)
    # Find datetime column
    ts_col = None
    for c in df.columns:
        if "datetime" in str(c).lower() or "date" in str(c).lower() or "time" in str(c).lower():
            ts_col = c; break
    ...
    volt_col = None
    for c in df.columns:
        if "volt" in str(c).lower(): volt_col = c; break
    if volt_col is None: volt_col = df.select_dtypes("number").columns[0]
    df[volt_col] = pd.to_numeric(df[volt_col], errors="coerce")
    # Aggregate: daily mean voltage across all machines
    df = df.set_index(ts_col)
    daily = df[volt_col].resample("D").mean().dropna().to_frame("volt")
