...
Data: Auto-downloaded at runtime via Kaggle (sumanthvrao/daily-climate-time-series-data)
       Daily humidity (rainfall proxy) from DailyDelhiClimateTrain.csv.
"""
...
def load_data():
    ...
    hum_col = None
    for c in df.columns:
        if "humidity" in str(c).lower(): hum_col = c; break
    if hum_col is None: hum_col = df.select_dtypes("number").columns[1] if len(df.select_dtypes("number").columns) > 1 else df.select_dtypes("number").columns[0]
    df[hum_col] = pd.to_numeric(df[hum_col], errors="coerce")
    daily = df[[date_col, hum_col]].copy()
    daily.columns = ["day", "humidity"]
