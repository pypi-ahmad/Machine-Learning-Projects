...
Data: Auto-downloaded at runtime via Kaggle (rohanrao/air-quality-data-in-india)
       Daily PM2.5 from city_day.csv, filtered to Delhi.
"""
...
def load_data():
    ...
    _target_file = None
    _csvs = _glob.glob(os.path.join(_data_dir, "**", "city_day.csv"), recursive=True)
    ...
    if _target_file is None:
        ...
        if not _all:
            ...
            _api.dataset_download_files("rohanrao/air-quality-data-in-india", path=_data_dir, unzip=True)
            _csvs = _glob.glob(os.path.join(_data_dir, "**", "city_day.csv"), recursive=True)
            if _csvs: _target_file = _csvs[0]
            print("Downloaded rohanrao/air-quality-data-in-india from Kaggle")
        if _target_file is None:
            _all = _glob.glob(os.path.join(_data_dir, "**", "*.csv"), recursive=True)
            _target_file = sorted(_all, key=os.path.getsize, reverse=True)[0]
    ...
    # Filter to Delhi if City column exists
    city_col = None
    for c in df.columns:
        if "city" in str(c).lower(): city_col = c; break
    if city_col and "Delhi" in df[city_col].unique():
        df = df[df[city_col] == "Delhi"].copy()
    # Find PM2.5 column
    pm_col = None
    for c in df.columns:
        if "pm2" in str(c).lower() or "pm25" in str(c).lower().replace(".", ""): pm_col = c; break
    if pm_col is None:
        for c in df.select_dtypes("number").columns:
            if "pm" in str(c).lower(): pm_col = c; break
    if pm_col is None: pm_col = df.select_dtypes("number").columns[0]
    df[pm_col] = pd.to_numeric(df[pm_col], errors="coerce")
    # Build daily series
    daily = df[[date_col, pm_col]].copy()
    daily.columns = ["day", "PM2.5"]
    daily = daily.sort_values("day").set_index("day")
    daily = daily.resample("D").mean()  # fill gaps
    # Interpolate missing values (PM2.5 often has gaps)
    daily["PM2.5"] = daily["PM2.5"].interpolate(method="linear", limit=7)
    daily = daily.dropna()
    target = TARGET
