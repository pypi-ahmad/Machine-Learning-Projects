...
Data: Auto-downloaded at runtime via Kaggle (lakshmi25npathi/bike-sharing-dataset)
"""
...
def load_data():
    ...
    _fp = os.path.join(_data_dir, "hour.csv")
    if not os.path.exists(_fp):
        ...
        _matches = _glob.glob(os.path.join(_data_dir, "**", "hour.csv"), recursive=True)
        if _matches: _fp = _matches[0]
        print("Downloaded lakshmi25npathi/bike-sharing-dataset from Kaggle")
    df = pd.read_csv(_fp)
    # Build datetime index from dteday + hr
    df["datetime"] = pd.to_datetime(df["dteday"]) + pd.to_timedelta(df["hr"], unit="h")
    df = df.sort_values("datetime").set_index("datetime")
    target = TARGET
    if target not in df.columns:
        for c in df.select_dtypes("number").columns:
            if any(kw in str(c).lower() for kw in ["cnt", "count", "total", "demand"]): target = c; break
        else: target = df.select_dtypes("number").columns[-1]
...
def forecast(df, target):
    ...
    try:
        ...
        ts_df = pd.DataFrame({"item_id": ["s"]*split, "timestamp": pd.date_range("2020-01-01", periods=split, freq="h"), "target": train})
    ...
    try:
        ...
        y_pred = np.median(pipe.predict(torch.tensor(train[-2000:], dtype=torch.float32), HORIZON)[0].numpy(), axis=0)[:len(test)]
    ...
    try:
        ...
        y_pred = np.median(pipe2.predict(torch.tensor(train[-2000:], dtype=torch.float32), HORIZON)[0].numpy(), axis=0)[:len(test)]
    ...
    try:
        ...
        y_pred, _ = tfm.forecast([train[-2000:]], [0]); y_pred = y_pred[0][:len(test)]
    ...
    try:
        ...
        y_pred = ARIMA(train[-2000:], order=(5,1,0)).fit().forecast(steps=HORIZON)[:len(test)]
    ...
    try:
        ...
        p_df = pd.DataFrame({"ds": pd.date_range("2020-01-01", periods=split, freq="h"), "y": train})
        m = Prophet(yearly_seasonality=True, weekly_seasonality=True, daily_seasonality=True); m.fit(p_df)
        y_pred = m.predict(m.make_future_dataframe(periods=HORIZON, freq="h"))["yhat"].values[-HORIZON:][:len(test)]
    ...
    lag_df["rolling_6"] = lag_df["y"].rolling(6).mean(); lag_df["rolling_24"] = lag_df["y"].rolling(24).mean()
    lag_df["rolling_168"] = lag_df["y"].rolling(168).mean()
    lag_df["diff_1"] = lag_df["y"].diff(1); lag_df["diff_24"] = lag_df["y"].diff(24)
    lag_df = lag_df.dropna(); offset = max(lags) + 168
    ...
    ax.plot(range(len(train[-500:])), train[-500:], alpha=0.5, label="Train (last 500)")
    ax.plot(range(len(train[-500:]), len(train[-500:])+len(test)), test, linewidth=2, label="Actual")
    for name, yp in results.items(): ax.plot(range(len(train[-500:]), len(train[-500:])+len(yp)), yp, "--", label=name)
...
def run_eda(df, target, save_dir):
    ...
    if target in df.columns:
        ...
        try:
            from statsmodels.tsa.seasonal import seasonal_decompose; s = df[target].dropna(); freq = min(24, len(s)//3)
            if len(s) > 2*freq:
