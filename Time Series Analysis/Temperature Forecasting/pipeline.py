...
Data: Auto-downloaded at runtime via Kaggle (sumanthvrao/daily-climate-time-series-data)
       Daily mean temperature in Delhi (DailyDelhiClimateTrain.csv).
"""
...
def load_data():
    ...
    _target_file = None
    _csvs = _glob.glob(os.path.join(_data_dir, "**", "DailyDelhiClimateTrain.csv"), recursive=True)
    if _csvs: _target_file = _csvs[0]
    if _target_file is None:
        _all = _glob.glob(os.path.join(_data_dir, "**", "*.csv"), recursive=True)
        if not _all:
            from kaggle.api.kaggle_api_extended import KaggleApi
            _api = KaggleApi(); _api.authenticate()
            _api.dataset_download_files("sumanthvrao/daily-climate-time-series-data", path=_data_dir, unzip=True)
            _csvs = _glob.glob(os.path.join(_data_dir, "**", "DailyDelhiClimateTrain.csv"), recursive=True)
            if _csvs: _target_file = _csvs[0]
            print("Downloaded sumanthvrao/daily-climate-time-series-data from Kaggle")
        if _target_file is None:
            _all = _glob.glob(os.path.join(_data_dir, "**", "*.csv"), recursive=True)
            # Prefer train file
            for f in _all:
                if "train" in os.path.basename(f).lower(): _target_file = f; break
            if _target_file is None: _target_file = sorted(_all, key=os.path.getsize, reverse=True)[0]
    df = pd.read_csv(_target_file)
    # Find date column
    date_col = None
    for c in df.columns:
    ...
    # Find meantemp column
    temp_col = None
    for c in df.columns:
        if "meantemp" in str(c).lower() or "mean_temp" in str(c).lower(): temp_col = c; break
    if temp_col is None:
        for c in df.select_dtypes("number").columns:
            if "temp" in str(c).lower(): temp_col = c; break
    if temp_col is None: temp_col = df.select_dtypes("number").columns[0]
    df[temp_col] = pd.to_numeric(df[temp_col], errors="coerce")
    daily = df[[date_col, temp_col]].copy()
    daily.columns = ["day", "meantemp"]
...
def forecast(df, target):
    ...
    try:
        ...
        m = Prophet(yearly_seasonality=True, weekly_seasonality=False, daily_seasonality=False); m.fit(p_df)
...
def run_eda(df, target, save_dir):
    ...
    if target in df.columns:
        ...
        try:
            from statsmodels.tsa.seasonal import seasonal_decompose; s = df[target].dropna()
            freq = min(365, len(s)//3) if len(s) > 730 else min(max(7, len(s)//10), 365)
            if len(s) > 2*freq:
