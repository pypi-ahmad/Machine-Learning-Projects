...
Data: Auto-downloaded at runtime via Kaggle (sazidthe1/river-water-flow)
       Daily river water flow / discharge measurements.
"""
...
def load_data():
    ...
    if not _csvs:
        ...
        _api.dataset_download_files("sazidthe1/river-water-flow", path=_data_dir, unzip=True)
        _csvs = _glob.glob(os.path.join(_data_dir, "**", "*.csv"), recursive=True)
        print("Downloaded sazidthe1/river-water-flow from Kaggle")
    _fp = sorted(_csvs, key=os.path.getsize, reverse=True)[0]
    df = pd.read_csv(_fp)
    # Find date column
    date_col = None
    for c in df.columns:
        if "date" in str(c).lower() or "time" in str(c).lower(): date_col = c; break
    ...
    for c in df.select_dtypes("number").columns:
        cl = str(c).lower()
        if any(kw in cl for kw in ["flow", "discharge", "streamflow", "runoff", "water"]): flow_col = c; break
    if flow_col is None: flow_col = df.select_dtypes("number").columns[-1]
    df[flow_col] = pd.to_numeric(df[flow_col], errors="coerce")
    daily = df[[date_col, flow_col]].copy()
    daily.columns = ["day", "flow"]
    daily = daily.sort_values("day").set_index("day")
    # Resample to daily to handle any gaps
    daily = daily.resample("D").mean()
    daily["flow"] = daily["flow"].interpolate(method="linear", limit=7)
    daily = daily.dropna()
    target = TARGET
