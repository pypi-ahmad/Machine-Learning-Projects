...
Data: Auto-downloaded at runtime via Kaggle (chirag19/air-passengers)
"""
...
def load_data():
    ...
    if not _csvs:
        ...
        _api.dataset_download_files("chirag19/air-passengers", path=_data_dir, unzip=True)
        _csvs = _glob.glob(os.path.join(_data_dir, "**", "*.csv"), recursive=True)
        print("Downloaded chirag19/air-passengers from Kaggle")
    df = pd.read_csv(_csvs[0])
    target = TARGET
    if target not in df.columns:
        for c in df.select_dtypes("number").columns:
            if any(kw in str(c).lower() for kw in ["passenger", "pax", "count", "total"]): target = c; break
        else: target = df.select_dtypes("number").columns[-1]
    for c in df.columns:
        if "month" in str(c).lower() or "date" in str(c).lower() or "time" in str(c).lower():
...
def forecast(df, target):
    ...
    try:
        ...
        ts_df = pd.DataFrame({"item_id": ["s"]*split, "timestamp": pd.date_range("1949-01-01", periods=split, freq="MS"), "target": train})
    ...
    try:
        ...
        p_df = pd.DataFrame({"ds": pd.date_range("1949-01-01", periods=split, freq="MS"), "y": train})
        m = Prophet(yearly_seasonality=True, weekly_seasonality=False, daily_seasonality=False); m.fit(p_df)
        y_pred = m.predict(m.make_future_dataframe(periods=HORIZON, freq="MS"))["yhat"].values[-HORIZON:][:len(test)]
    ...
    lag_df["rolling_3"] = lag_df["y"].rolling(3).mean(); lag_df["rolling_6"] = lag_df["y"].rolling(6).mean()
    lag_df["rolling_12"] = lag_df["y"].rolling(12).mean()
    lag_df["diff_1"] = lag_df["y"].diff(1); lag_df["diff_12"] = lag_df["y"].diff(12)
    lag_df = lag_df.dropna(); offset = max(lags) + 12
...
def run_eda(df, target, save_dir):
    ...
    if target in df.columns:
        ...
        try:
            from statsmodels.tsa.seasonal import seasonal_decompose; s = df[target].dropna(); freq = 12
            if len(s) > 2*freq:
