...
Data: Auto-downloaded at runtime via Kaggle (dgomonov/new-york-city-airbnb-open-data)
       Airbnb review dates are aggregated to daily review counts as a tourism demand proxy.
"""
...
def load_data():
    ...
    if not _csvs:
        ...
        _api.dataset_download_files("dgomonov/new-york-city-airbnb-open-data", path=_data_dir, unzip=True)
        _csvs = _glob.glob(os.path.join(_data_dir, "**", "*.csv"), recursive=True)
        print("Downloaded dgomonov/new-york-city-airbnb-open-data from Kaggle")
    _fp = sorted(_csvs, key=os.path.getsize, reverse=True)[0]
    df = pd.read_csv(_fp)
    # Find last_review column and build daily review count proxy
    review_col = None
    for c in df.columns:
        if "last_review" in str(c).lower() or "review" in str(c).lower() and "date" in str(c).lower():
            review_col = c; break
    if review_col is None:
        for c in df.columns:
            if "review" in str(c).lower(): review_col = c; break
    if review_col is None: review_col = df.columns[0]
    df[review_col] = pd.to_datetime(df[review_col], errors="coerce")
    df = df.dropna(subset=[review_col])
    # Aggregate: count listings with a review on each day (proxy for tourism activity)
    daily = df.groupby(df[review_col].dt.date).size().reset_index(name="review_count")
    daily.columns = ["day", "review_count"]
    ...
    # Trim early sparse period (keep last 3 years at most)
    if len(daily) > 1095:
        daily = daily.tail(1095)
    target = TARGET
