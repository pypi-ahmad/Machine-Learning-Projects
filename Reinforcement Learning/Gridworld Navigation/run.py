#!/usr/bin/env python3
"""
Gridworld Navigation — DQN
============================
Train a Deep Q-Network agent on a deterministic grid-world using
**FrozenLake-v1** (8×8, ``is_slippery=False``) from OpenAI Gymnasium.

With slipperiness disabled, the environment behaves as a classic
deterministic grid-world where the agent must navigate from start (S)
to goal (G) while avoiding holes (H).  The 64 discrete states are
one-hot encoded before being fed into the DQN.
...
MAP_SIZE = "8x8"
STATE_DIM = 64            # one-hot encoded (8×8 grid)
...
def train_dqn(device: torch.device, output_dir: Path, num_episodes: int = NUM_EPISODES, batch_size: int = BATCH_SIZE):
    ...
    env = gym.make(ENV_NAME, map_name=MAP_SIZE, is_slippery=False)
    ...
    ax.set_title(f"DQN Training on {ENV_NAME} ({MAP_SIZE}, deterministic)")
    ax.legend()
    fig.tight_layout()
    ...
    # Model weights
    model_path = output_dir / "dqn_gridworld.pth"
...
def evaluate_policy(policy_net, env, device, num_episodes=100, seed=42):
    ...
    for ep in range(num_episodes):
        ...
        if total_reward > 0:  # reached goal
            successes += 1
    return {
...
def main() -> None:
    setup_logging()
    args = parse_common_args("DQN on FrozenLake 8x8 deterministic gridworld")
    set_seed(args.seed)
    configure_cuda_allocator()
    ...
    dataset_prompt(
        name="OpenAI Gymnasium FrozenLake-v1 (8×8, deterministic gridworld)",
        ...
        notes="Built-in Gymnasium environment — no download required. "
              "Using 8×8 map with is_slippery=False as a deterministic gridworld.",
    )

    logger.info("Training DQN on %s (%s, deterministic) for %d episodes …", ENV_NAME, MAP_SIZE, train_episodes)
    ...
    eval_env = gym.make(ENV_NAME, map_name=MAP_SIZE, is_slippery=False)
    ...
    write_split_manifest(
        output_dir,
        dataset_fp={"env_name": ENV_NAME, "map_size": MAP_SIZE, "is_slippery": False},
