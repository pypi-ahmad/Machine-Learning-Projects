#!/usr/bin/env python3
"""
Reinforcement Learning for Taxis — DQN
========================================
Train a Deep Q-Network agent on the **Taxi-v3** environment from
OpenAI Gymnasium.

Taxi-v3 has a discrete observation space of 500 states.  We one-hot
encode each state so it can be fed into a standard feed-forward DQN.
...
# ── Hyper-parameters ─────────────────────────────────────────
ENV_NAME = "Taxi-v3"
STATE_DIM = 500          # one-hot encoded discrete state
ACTION_DIM = 6
NUM_EPISODES = 1000
LR = 1e-3
GAMMA = 0.99
...
def train_dqn(device: torch.device, output_dir: Path, num_episodes: int = NUM_EPISODES, batch_size: int = BATCH_SIZE):
    ...
    # Model weights
    model_path = output_dir / "dqn_taxi.pth"
...
def evaluate_policy(policy_net, env, device, num_episodes=100, seed=42):
    ...
    for ep in range(num_episodes):
        ...
        if total_reward > 0:  # passenger delivered successfully
            successes += 1
    return {
...
def main() -> None:
    setup_logging()
    args = parse_common_args("DQN on Taxi-v3")
    set_seed(args.seed)
    configure_cuda_allocator()
    ...
    dataset_prompt(
        name="OpenAI Gymnasium Taxi-v3",
        official_links=["https://gymnasium.farama.org/environments/toy_text/taxi/"],
