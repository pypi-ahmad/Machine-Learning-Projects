...
Lunar Landing — DQN
====================
Train a Deep Q-Network agent on the **LunarLander-v3** environment
from OpenAI Gymnasium.

The agent observes an 8-dimensional continuous state vector and
selects from 4 discrete actions (do nothing, fire left, main, right
engine).
...
def train_dqn(device: torch.device, output_dir: Path, num_episodes: int = NUM_EPISODES, batch_size: int = BATCH_SIZE):
    ...
    try:
        ...
        missing_dependency_metrics(
            output_dir,
            missing=["Box2D"],
            install_cmd='pip install swig && pip install "gymnasium[box2d]"',
        )
    ...
    # Model weights
    model_path = output_dir / "dqn_lunar_lander.pth"
...
def evaluate_policy(policy_net, env, device, num_episodes=100, seed=42):
    ...
    for ep in range(num_episodes):
        ...
        if total_reward >= 200:  # successful landing
            successes += 1
    return {
...
def main() -> None:
    setup_logging()
    args = parse_common_args("DQN on LunarLander-v3")
    set_seed(args.seed)
    configure_cuda_allocator()
    ...
    dataset_prompt(
        name="OpenAI Gymnasium LunarLander-v3",
        official_links=["https://gymnasium.farama.org/environments/box2d/lunar_lander/"],
