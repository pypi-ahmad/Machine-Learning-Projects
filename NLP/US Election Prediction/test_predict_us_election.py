"""Tests for NLP Projecct 7.Predict_US_election"""
import pytest
import pandas as pd
import numpy as np
import re
...
def data_dir():
    ...
    d = root / "data" / "NLP Projecct 7.Predict_US_election"
    if not d.exists():
...
def df(data_dir):
    ...
    return pd.read_csv(data_dir / 'Trumpall2.csv')


@pytest.fixture(scope="module")
def df_extra(data_dir):
    """Load secondary dataset."""
    return pd.read_csv(data_dir / 'Bidenall2.csv')
...
class TestDataLoading:
    def test_file_exists(self, data_dir):
        assert (data_dir / "Trumpall2.csv").exists()
    ...
    def test_extra_file_exists(self, data_dir):
        assert (data_dir / "Bidenall2.csv").exists()

    def test_extra_loads(self, df_extra):
        assert len(df_extra) > 0
...
class TestPreprocessing:
    ...
    def test_basic_text_cleaning(self, df):
        sample = str(df["text"].dropna().iloc[0])
...
class TestModel:
    def test_tfidf_vectorizer(self, df):
        ...
        texts = df["text"].astype(str).head(200)
    ...
    def test_model_fit(self, df):
        ...
        subset = df[["text", "user"]].dropna().head(200)
        vec = TfidfVectorizer(max_features=100)
        X = vec.fit_transform(subset["text"].astype(str))
        y = subset["user"]
        model = MultinomialNB()
        model.fit(X, y)
