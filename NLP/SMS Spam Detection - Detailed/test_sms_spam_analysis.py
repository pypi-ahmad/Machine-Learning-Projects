"""Tests for NLP Projects 31 - SMS Spam Detection Analysis"""
import pytest
import pandas as pd
import numpy as np
import re
...
def data_dir():
    ...
    d = root / "data" / "NLP Projects 31 - SMS Spam Detection Analysis"
    if not d.exists():
...
def df(data_dir):
    ...
    return pd.read_csv(data_dir / 'spam.csv')
...
class TestDataLoading:
    def test_file_exists(self, data_dir):
        assert (data_dir / "spam.csv").exists()
...
class TestModel:
    def test_tfidf_vectorizer(self, df):
        ...
        texts = df["v2"].astype(str).head(200)
    ...
    def test_model_fit(self, df):
        ...
        subset = df[["v2", "v1"]].dropna().head(200)
        vec = TfidfVectorizer(max_features=100)
        X = vec.fit_transform(subset["v2"].astype(str))
        y = subset["v1"]
        model = MultinomialNB()
        model.fit(X, y)
