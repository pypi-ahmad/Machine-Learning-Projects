"""Tests for NLP Projects 27 - Amazon Alexa Review Sentiment Analysis"""
import pytest
import pandas as pd
import numpy as np
import re
...
def data_dir():
    ...
    d = root / "data" / "NLP Projects 27 - Amazon Alexa Review Sentiment Analysis"
    if not d.exists():
...
def df(data_dir):
    ...
    return pd.read_csv(data_dir / 'amazon_alexa.tsv', delimiter='\t')
...
class TestDataLoading:
    def test_file_exists(self, data_dir):
        assert (data_dir / "amazon_alexa.tsv").exists()
    ...
    def test_expected_columns(self, df):
        for col in ['rating', 'variation', 'verified_reviews', 'feedback']:
            assert col in df.columns, f"Missing column: {col}"

    def test_no_fully_null_columns(self, df):
        key_cols = ['rating', 'variation', 'verified_reviews', 'feedback']
        for col in key_cols:
...
class TestModel:
    def test_tfidf_vectorizer(self, df):
        ...
        texts = df["verified_reviews"].astype(str).head(200)
    ...
    def test_model_fit(self, df):
        ...
        subset = df[["verified_reviews", "feedback"]].dropna().head(200)
        vec = TfidfVectorizer(max_features=100)
        X = vec.fit_transform(subset["verified_reviews"].astype(str))
        y = subset["feedback"]
        model = MultinomialNB()
        model.fit(X, y)
