"""Tests for NLP Projects 26 - IMDB Sentiment Analysis using Deep Learning"""
import pytest
import pandas as pd
import numpy as np
import re
...
def data_dir():
    ...
    d = root / "data" / "NLP Projects 26 - IMDB Sentiment Analysis using Deep Learning"
    if not d.exists():
...
def df(data_dir):
    ...
    return pd.read_csv(data_dir / 'IMDB Dataset.csv')
...
class TestDataLoading:
    def test_file_exists(self, data_dir):
        assert (data_dir / "IMDB Dataset.csv").exists()
...
class TestModel:
    ...
    def test_model_fit(self, df):
        ...
        subset = df[["review", "sentiment"]].dropna().head(200)
        vec = TfidfVectorizer(max_features=100)
