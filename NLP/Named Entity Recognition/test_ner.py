"""Tests for NLP Projecct 2.Named Entity Recognition"""
import pytest
import pandas as pd
import numpy as np
import re
...
def data_dir():
    ...
    d = root / "data" / "NLP Projecct 2.Named Entity Recognition"
    if not d.exists():
...
def df(data_dir):
    ...
    return pd.read_csv(data_dir / 'ner_dataset.csv', encoding='unicode_escape')
...
class TestDataLoading:
    def test_file_exists(self, data_dir):
        assert (data_dir / "ner_dataset.csv").exists()
    ...
    def test_expected_columns(self, df):
        for col in ['Sentence #', 'Word', 'POS', 'Tag']:
            assert col in df.columns, f"Missing column: {col}"

    def test_no_fully_null_columns(self, df):
        key_cols = ['Sentence #', 'Word', 'POS', 'Tag']
        for col in key_cols:
...
class TestModel:
    def test_tfidf_vectorizer(self, df):
        ...
        texts = df["Word"].astype(str).head(500)
    ...
    def test_model_fit(self, df):
        ...
        subset = df[["Word", "Tag"]].dropna().head(500)
        vec = TfidfVectorizer(max_features=100)
        X = vec.fit_transform(subset["Word"].astype(str))
        y = subset["Tag"]
        model = MultinomialNB()
        model.fit(X, y)
