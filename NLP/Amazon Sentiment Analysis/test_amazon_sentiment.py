"""Tests for NLP Projecct 20.Amazon Sentiment Analysis"""
import pytest
import pandas as pd
import numpy as np
import re
...
def data_dir():
    ...
    d = root / "data" / "NLP Projecct 20.Amazon Sentiment Analysis"
    if not d.exists():
...
def df(data_dir):
    ...
    return pd.read_csv(data_dir / 'amazon.csv', low_memory=False)
...
class TestDataLoading:
    def test_file_exists(self, data_dir):
        assert (data_dir / "amazon.csv").exists()
    ...
    def test_expected_columns(self, df):
        for col in ['name', 'reviews.rating', 'reviews.text']:
            assert col in df.columns, f"Missing column: {col}"

    def test_no_fully_null_columns(self, df):
        key_cols = ['name', 'reviews.rating', 'reviews.text']
        for col in key_cols:
...
class TestModel:
    def test_tfidf_vectorizer(self, df):
        ...
        texts = df["reviews.text"].astype(str).head(200)
    ...
    def test_model_fit(self, df):
        ...
        subset = df[["reviews.text", "reviews.rating"]].dropna().head(200)
        vec = TfidfVectorizer(max_features=100)
        X = vec.fit_transform(subset["reviews.text"].astype(str))
        y = subset["reviews.rating"]
        model = MultinomialNB()
        model.fit(X, y)
