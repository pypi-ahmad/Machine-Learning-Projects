"""Tests for NLP Projects 25 - Twitter Us Airline Sentiment Analysis"""
import pytest
import pandas as pd
import numpy as np
import re
...
def data_dir():
    ...
    d = root / "data" / "NLP Projects 25 - Twitter Us Airline Sentiment Analysis"
    if not d.exists():
...
def df(data_dir):
    ...
    return pd.read_csv(data_dir / 'Tweets.csv')
...
class TestDataLoading:
    def test_file_exists(self, data_dir):
        assert (data_dir / "Tweets.csv").exists()
    ...
    def test_no_fully_null_columns(self, df):
        key_cols = ['airline_sentiment', 'airline', 'text']
        for col in key_cols:
...
class TestPreprocessing:
    ...
    def test_label_has_multiple_classes(self, df):
        assert df["airline_sentiment"].nunique() >= 2
...
class TestModel:
    ...
    def test_model_fit(self, df):
        ...
        subset = df[["text", "airline_sentiment"]].dropna().head(200)
        vec = TfidfVectorizer(max_features=100)
        X = vec.fit_transform(subset["text"].astype(str))
        y = subset["airline_sentiment"]
        model = MultinomialNB()
        model.fit(X, y)
