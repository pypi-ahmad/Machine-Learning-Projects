"""Tests for NLP Projecct 17.Twitter Sentiment Analysis"""
import pytest
import pandas as pd
import numpy as np
import re
...
def data_dir():
    ...
    d = root / "data" / "NLP Projecct 17.Twitter Sentiment Analysis"
    if not d.exists():
...
def df(data_dir):
    ...
    return pd.read_csv(data_dir / 'Sentiment.csv')
...
class TestDataLoading:
    def test_file_exists(self, data_dir):
        assert (data_dir / "Sentiment.csv").exists()
    ...
    def test_expected_columns(self, df):
        for col in ['id', 'candidate', 'sentiment', 'text']:
            assert col in df.columns, f"Missing column: {col}"

    def test_no_fully_null_columns(self, df):
        key_cols = ['id', 'candidate', 'sentiment', 'text']
        for col in key_cols:
...
class TestModel:
    ...
    def test_model_fit(self, df):
        ...
        subset = df[["text", "sentiment"]].dropna().head(200)
        vec = TfidfVectorizer(max_features=100)
