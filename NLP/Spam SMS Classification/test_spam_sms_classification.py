"""Tests for NLP Project 23. - Spam SMS Classification"""
import pytest
import pandas as pd
import numpy as np
import re
...
def data_dir():
    ...
    d = root / "data" / "NLP Project 23. - Spam SMS Classification"
    if not d.exists():
...
def df(data_dir):
    ...
    return pd.read_csv(data_dir / 'Spam SMS Collection', sep='\t', names=['label', 'message'])
...
class TestDataLoading:
    def test_file_exists(self, data_dir):
        assert (data_dir / "Spam SMS Collection").exists()
...
class TestModel:
    def test_tfidf_vectorizer(self, df):
        ...
        texts = df["message"].astype(str).head(200)
    ...
    def test_model_fit(self, df):
        ...
        subset = df[["message", "label"]].dropna().head(200)
        vec = TfidfVectorizer(max_features=100)
        X = vec.fit_transform(subset["message"].astype(str))
        y = subset["label"]
        model = MultinomialNB()
        model.fit(X, y)
