"""Tests for NLP Projecct 6.Autocorrect"""
import pytest
import re
...
def data_dir():
    ...
    d = root / "data" / "NLP Projecct 6.Autocorrect"
    if not d.exists():
...
class TestDataLoading:
    def test_file_exists(self, data_dir):
        assert (data_dir / "moby.txt").exists()

    def test_file_not_empty(self, data_dir):
        assert (data_dir / "moby.txt").stat().st_size > 0
