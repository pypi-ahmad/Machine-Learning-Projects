"""Tests for NLP Projecct 15.FakeNews Detection Model"""
import pytest
import pandas as pd
import numpy as np
import re
...
def data_dir():
    ...
    d = root / "data" / "NLP Projecct 15.FakeNews Detection Model"
    if not d.exists():
...
class TestDataLoading:
    ...
    def test_expected_columns(self, df):
        for col in ['id', 'title', 'author', 'text', 'label']:
            assert col in df.columns, f"Missing column: {col}"

    def test_no_fully_null_columns(self, df):
        key_cols = ['id', 'title', 'author', 'text', 'label']
        for col in key_cols:
...
class TestModel:
    def test_tfidf_vectorizer(self, df):
        ...
        texts = df["text"].fillna("").astype(str).head(200)
    ...
    def test_model_fit(self, df):
        ...
        subset = df[["text", "label"]].dropna().head(200)
        vec = TfidfVectorizer(max_features=100)
