"""
Tests for: Project 1- SONAR Rock vs Mine Prediction
...
DATA_DIR = Path(__file__).resolve().parent.parent / "data" / "sonar_rock_vs_mine_prediction"
pytestmark = []
...
class TestDataLoading:
    ...
    def test_load_without_error(self):
        ...
        if not fpath.exists():
            pytest.skip(f"Data file not found: {fpath}")
        try:
            df = pd.read_csv(DATA_DIR / "sonar data.csv")
        except (UnicodeDecodeError, ValueError):
            df = pd.read_csv(DATA_DIR / "sonar data.csv", encoding="latin-1")
