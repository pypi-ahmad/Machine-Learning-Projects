"""
Tests for: Machine Learning Project 13 - Gold Price Prediction
...
DATA_DIR = Path(__file__).resolve().parent.parent / "data" / "gold_price_prediction"
pytestmark = []
...
class TestDataLoading:
    ...
    def test_load_without_error(self):
        ...
        if not fpath.exists():
            pytest.skip(f"Data file not found: {fpath}")
        try:
            df = pd.read_csv(DATA_DIR / "gold_price.csv")
        except (UnicodeDecodeError, ValueError):
            df = pd.read_csv(DATA_DIR / "gold_price.csv", encoding="latin-1")
