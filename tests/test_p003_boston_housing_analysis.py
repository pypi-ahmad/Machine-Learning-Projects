"""
Tests for: Machine Learning Project 3 - Boston Housing Analysis
...
DATA_DIR = Path(__file__).resolve().parent.parent / "data" / "boston_housing_analysis"
pytestmark = []
...
class TestDataLoading:
    ...
    def test_load_without_error(self):
        ...
        if not fpath.exists():
            pytest.skip(f"Data file not found: {fpath}")
        try:
            df = pd.read_csv(DATA_DIR / "housing.data", header=None, sep=r"\s+")
        except (UnicodeDecodeError, ValueError):
            df = pd.read_csv(DATA_DIR / "housing.data", encoding="latin-1", header=None, sep=r"\s+")
