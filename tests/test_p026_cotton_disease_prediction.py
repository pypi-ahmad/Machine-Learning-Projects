"""
Tests for: Machine Learning Project 26 - Cotton_Disease_Prediction
...
DATA_DIR = Path(__file__).resolve().parent.parent / "data" / "cotton_disease_prediction"
pytestmark = []
...
class TestDataLoading:
    ...
    def test_load_without_error(self):
        ...
        if not fpath.exists():
            pytest.skip(f"Data file not found: {fpath}")
        try:
            df = pd.read_csv(DATA_DIR / "(3 files)")
        except (UnicodeDecodeError, ValueError):
            df = pd.read_csv(DATA_DIR / "(3 files)", encoding="latin-1")
