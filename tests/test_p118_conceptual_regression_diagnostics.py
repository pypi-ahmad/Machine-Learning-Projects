"""
Tests for: Machine Learning Projects 118 - (Conceptual) Regression Diagnostics
...
DATA_DIR = Path(__file__).resolve().parent.parent / "data" / "conceptual_regression_diagnostics"
pytestmark = []
...
class TestDataLoading:
    ...
    def test_load_without_error(self):
        ...
        if not fpath.exists():
            pytest.skip(f"Data file not found: {fpath}")
        try:
            df = pd.read_csv(DATA_DIR / "Guerry.csv")
        except (UnicodeDecodeError, ValueError):
            df = pd.read_csv(DATA_DIR / "Guerry.csv", encoding="latin-1")
