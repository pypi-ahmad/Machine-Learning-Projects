"""
Tests for: Machine Learning Project 37 - Text Summarization(Medium)
...
DATA_DIR = Path(__file__).resolve().parent.parent / "data" / "text_summarizationmedium"
pytestmark = []
...
class TestDataLoading:
    ...
    def test_load_without_error(self):
        ...
        if not fpath.exists():
            pytest.skip(f"Data file not found: {fpath}")
        try:
            df = pd.read_csv(DATA_DIR / "tennis.csv")
        except (UnicodeDecodeError, ValueError):
            df = pd.read_csv(DATA_DIR / "tennis.csv", encoding="latin-1")
