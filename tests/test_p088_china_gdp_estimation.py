"""
Tests for: Machine Learning Projects  88 -China GDP Estimation
...
DATA_DIR = Path(__file__).resolve().parent.parent / "data" / "china_gdp_estimation"
pytestmark = []
...
class TestDataLoading:
    ...
    def test_load_without_error(self):
        ...
        if not fpath.exists():
            pytest.skip(f"Data file not found: {fpath}")
        try:
            df = pd.read_csv(DATA_DIR / "china_gdp.csv")
        except (UnicodeDecodeError, ValueError):
            df = pd.read_csv(DATA_DIR / "china_gdp.csv", encoding="latin-1")
