"""
Tests for: Machine Learning Project 44 - XgBoost ALgorithm
...
DATA_DIR = Path(__file__).resolve().parent.parent / "data" / "xgboost_algorithm"
pytestmark = []
...
class TestDataLoading:
    ...
    def test_load_without_error(self):
        ...
        if not fpath.exists():
            pytest.skip(f"Data file not found: {fpath}")
        try:
            df = pd.read_csv(DATA_DIR / "melb_data.csv")
        except (UnicodeDecodeError, ValueError):
            df = pd.read_csv(DATA_DIR / "melb_data.csv", encoding="latin-1")
