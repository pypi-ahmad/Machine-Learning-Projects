"""
Tests for: Machine Learning Project 48 - Car Price Prediction
...
DATA_DIR = Path(__file__).resolve().parent.parent / "data" / "car_price_prediction"
pytestmark = []
...
class TestDataLoading:
    ...
    def test_load_without_error(self):
        ...
        if not fpath.exists():
            pytest.skip(f"Data file not found: {fpath}")
        try:
            df = pd.read_csv(DATA_DIR / "car_data.csv")
        except (UnicodeDecodeError, ValueError):
            df = pd.read_csv(DATA_DIR / "car_data.csv", encoding="latin-1")
