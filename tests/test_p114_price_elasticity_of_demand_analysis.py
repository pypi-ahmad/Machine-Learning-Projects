"""
Tests for: Machine Learning Projects 114 - Price Elasticity of Demand Analysis
...
DATA_DIR = Path(__file__).resolve().parent.parent / "data" / "price_elasticity_of_demand_analysis"
pytestmark = []
...
class TestDataLoading:
    ...
    def test_load_without_error(self):
        ...
        if not fpath.exists():
            pytest.skip(f"Data file not found: {fpath}")
        try:
            df = pd.read_csv(DATA_DIR / "beef.csv")
        except (UnicodeDecodeError, ValueError):
            df = pd.read_csv(DATA_DIR / "beef.csv", encoding="latin-1")
