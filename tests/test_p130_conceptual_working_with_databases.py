"""
Tests for: Machine Learning Projects 130 - (Conceptual) Working with Databases
...
DATA_DIR = Path(__file__).resolve().parent.parent / "data" / "conceptual_working_with_databases"
pytestmark = []
...
class TestDataLoading:
    ...
    def test_load_without_error(self):
        ...
        if not fpath.exists():
            pytest.skip(f"Data file not found: {fpath}")
        try:
            df = pd.read_csv(DATA_DIR / "iris_test.csv")
        except (UnicodeDecodeError, ValueError):
            df = pd.read_csv(DATA_DIR / "iris_test.csv", encoding="latin-1")
