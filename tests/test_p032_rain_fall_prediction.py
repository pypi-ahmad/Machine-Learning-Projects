"""
Tests for: Machine Learning Project 32 - Rain Fall Prediction
...
DATA_DIR = Path(__file__).resolve().parent.parent / "data" / "rain_fall_prediction"
pytestmark = []
...
class TestDataLoading:
    ...
    def test_load_without_error(self):
        ...
        if not fpath.exists():
            pytest.skip(f"Data file not found: {fpath}")
        try:
            df = pd.read_csv(DATA_DIR / "weatherAUS.csv")
        except (UnicodeDecodeError, ValueError):
            df = pd.read_csv(DATA_DIR / "weatherAUS.csv", encoding="latin-1")
