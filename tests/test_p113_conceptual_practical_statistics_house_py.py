"""
Tests for: Machine Learning Projects 113 - (Conceptual) Practical Statistics House Python_update
...
DATA_DIR = Path(__file__).resolve().parent.parent / "data" / "conceptual_practical_statistics_house_python_update"
pytestmark = []
...
class TestDataLoading:
    ...
    def test_load_without_error(self):
        ...
        if not fpath.exists():
            pytest.skip(f"Data file not found: {fpath}")
        try:
            df = pd.read_csv(DATA_DIR / "train (2).csv")
        except (UnicodeDecodeError, ValueError):
            df = pd.read_csv(DATA_DIR / "train (2).csv", encoding="latin-1")
