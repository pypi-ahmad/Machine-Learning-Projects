"""
Tests for: Machine Learning Projects 83 -Regression - First Innings Score Prediction - IPL
...
DATA_DIR = Path(__file__).resolve().parent.parent / "data" / "regression_-_first_innings_score_prediction_-_ipl"
pytestmark = []
...
class TestDataLoading:
    ...
    def test_load_without_error(self):
        ...
        if not fpath.exists():
            pytest.skip(f"Data file not found: {fpath}")
        try:
            df = pd.read_csv(DATA_DIR / "ipl.csv")
        except (UnicodeDecodeError, ValueError):
            df = pd.read_csv(DATA_DIR / "ipl.csv", encoding="latin-1")
