"""
Tests for: Machine Learning Projects 105 - Bayesian Statistics Python_PyMC3_ArviZ
...
DATA_DIR = Path(__file__).resolve().parent.parent / "data" / "bayesian_statistics_python_pymc3_arviz"
pytestmark = []
...
class TestDataLoading:
    ...
    def test_data_file_exists(self):
        ...
        fpath = DATA_DIR / "renfe_small.csv"
        if not fpath.exists():
    ...
    def test_load_without_error(self):
        ...
        if not fpath.exists():
            pytest.skip(f"Data file not found: {fpath}")
        try:
            df = pd.read_csv(DATA_DIR / "renfe_small.csv")
        except (UnicodeDecodeError, ValueError):
            df = pd.read_csv(DATA_DIR / "renfe_small.csv", encoding="latin-1")
