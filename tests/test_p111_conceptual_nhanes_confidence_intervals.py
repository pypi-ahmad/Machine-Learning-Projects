"""
Tests for: Machine Learning Projects 111 - (Conceptual) Nhanes_confidence_intervals
...
DATA_DIR = Path(__file__).resolve().parent.parent / "data" / "conceptual_nhanes_confidence_intervals"
pytestmark = []
...
class TestDataLoading:
    ...
    def test_load_without_error(self):
        ...
        if not fpath.exists():
            pytest.skip(f"Data file not found: {fpath}")
        try:
            df = pd.read_csv(DATA_DIR / "NHANES.csv")
        except (UnicodeDecodeError, ValueError):
            df = pd.read_csv(DATA_DIR / "NHANES.csv", encoding="latin-1")
