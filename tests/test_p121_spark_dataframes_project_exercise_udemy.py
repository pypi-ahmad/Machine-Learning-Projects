"""
Tests for: Machine Learning Projects 121 - Spark DataFrames Project Exercise_Udemy
...
DATA_DIR = Path(__file__).resolve().parent.parent / "data" / "spark_dataframes_project_exercise_udemy"
pytestmark = []
...
class TestDataLoading:
    ...
    def test_load_without_error(self):
        ...
        if not fpath.exists():
            pytest.skip(f"Data file not found: {fpath}")
        try:
            df = pd.read_csv(DATA_DIR / "WMT.csv")
        except (UnicodeDecodeError, ValueError):
            df = pd.read_csv(DATA_DIR / "WMT.csv", encoding="latin-1")
