"""
Tests for: Machine Learning Projects 116 - PySpark_Basic_DataFrame_Operations
...
DATA_DIR = Path(__file__).resolve().parent.parent / "data" / "pyspark_basic_dataframe_operations"
pytestmark = []
...
class TestDataLoading:
    ...
    def test_load_without_error(self):
        ...
        if not fpath.exists():
            pytest.skip(f"Data file not found: {fpath}")
        try:
            df = pd.read_csv(DATA_DIR / "apple.csv")
        except (UnicodeDecodeError, ValueError):
            df = pd.read_csv(DATA_DIR / "apple.csv", encoding="latin-1")
