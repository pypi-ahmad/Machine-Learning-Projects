...
st.caption("Synthetic data and illustrative rules only. Do not use this demo to approve, decline, or investigate real transactions.")
...
# ── Apply scores ──────────────────────────────────────────────────────────────
@st.cache_data
def score_all(data: pd.DataFrame) -> pd.DataFrame:
    rows = []
...
scored_df = score_all(df_all)

# ── UI ────────────────────────────────────────────────────────────────────────

tab1, tab2, tab3 = st.tabs(["Analyze transaction", "Transaction monitor", "Demo stats"])
...
with tab2:
    ...
    risk_filter = st.multiselect(
        "Filter by risk",
        ["High risk - review", "Medium risk - review", "Low risk"],
        default=["High risk - review", "Medium risk - review"],
    )
    filtered = scored_df[scored_df["label"].isin(risk_filter)] if risk_filter else scored_df
    st.dataframe(filtered.head(50), hide_index=True)

    c1, c2, c3 = st.columns(3)
    high = (scored_df["label"] == "High risk - review").sum()
    med = (scored_df["label"] == "Medium risk - review").sum()
...
with tab3:
    st.caption("These metrics compare the demo score with synthetic labels generated by the same simulation. They are not validation results for a real fraud model.")
