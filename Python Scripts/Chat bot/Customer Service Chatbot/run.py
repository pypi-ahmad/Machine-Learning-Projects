...
==================================================
Fine-tunes DistilBERT on a customer-service utterance dataset to classify
user messages into intent categories (e.g. cancel_order, track_refund, …).
...
TEXT_CANDIDATES = ["utterance", "text", "message", "sentence", "query", "question"]
LABEL_CANDIDATES = ["intent", "category", "label", "class", "tag", "topic"]
...
class TextDataset(Dataset):
    """PyTorch Dataset that wraps pre-tokenised text."""
...
def _find_column(df: pd.DataFrame, candidates: list[str], kind: str) -> str:
    """Return the first matching column name from *candidates*, else heuristic."""
    cols_lower = {c.lower(): c for c in df.columns}
    for cand in candidates:
        if cand in cols_lower:
            return cols_lower[cand]
    # Heuristic: first object column for text, second for label
...
def get_data(data_dir: Path) -> pd.DataFrame:
    """Download the dataset and return a DataFrame."""
    ds_path = download_kaggle_dataset(
        KAGGLE_SLUG, data_dir,
        dataset_name="Customer-Service Chatbot Dataset",
    )
...
def preprocess(df: pd.DataFrame):
    """Detect columns, encode labels, return (texts, labels, encoder)."""
    text_col = _find_column(df, TEXT_CANDIDATES, "text")
    label_col = _find_column(df, LABEL_CANDIDATES, "label")
    logger.info("Using text column: '%s'  |  label column: '%s'", text_col, label_col)
...
# ═════════════════════════════════════════════════════════════
def main():
    args = parse_common_args("Customer Service Chatbot — Intent Classification")
    setup_logging()
