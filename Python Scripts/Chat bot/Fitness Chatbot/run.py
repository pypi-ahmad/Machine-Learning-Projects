#!/usr/bin/env python3
"""
Fitness Chatbot — Intent Recognition
======================================
Fine-tunes DistilBERT on a chatbot intent-recognition dataset to classify
user messages into fitness-related intent tags.

The dataset typically has a JSON or CSV structure with
patterns (user text) and tags (intent labels).
...
KAGGLE_SLUG = "elvinagammed/chatbots-intent-recognition-dataset"
...
TEXT_CANDIDATES = ["patterns", "pattern", "text", "utterance", "message", "sentence", "query"]
LABEL_CANDIDATES = ["tag", "tags", "intent", "category", "label", "class"]
...
#  Data loading
# ═════════════════════════════════════════════════════════════
def _load_json_intents(path: Path) -> pd.DataFrame:
    """Load a typical intents JSON (tag → patterns list) into a flat DataFrame."""
    with open(path, encoding="utf-8") as f:
        data = json.load(f)
    rows = []
    # Handle multiple common JSON structures
    intents = data if isinstance(data, list) else data.get("intents", data.get("data", []))
    for entry in intents:
        tag = entry.get("tag") or entry.get("intent") or entry.get("label", "unknown")
        patterns = entry.get("patterns") or entry.get("text") or entry.get("examples", [])
        if isinstance(patterns, str):
            patterns = [patterns]
        for pat in patterns:
            rows.append({"text": pat, "tag": tag})
...
def get_data(data_dir: Path) -> pd.DataFrame:
    ds_path = download_kaggle_dataset(
        ...
        dataset_name="Chatbot Intent Recognition Dataset",
    )

    # Look for JSON files first (common for intent datasets)
    ...
    if jsons:
        ...
        logger.info("Loading JSON intent file: %s", chosen.name)
        df = _load_json_intents(chosen)
    elif csvs:
        chosen = max(csvs, key=lambda f: f.stat().st_size)
        df = pd.read_csv(chosen)
    else:
        raise FileNotFoundError(f"No JSON/CSV found in {ds_path}")
...
# ═════════════════════════════════════════════════════════════
def main():
    args = parse_common_args("Fitness Chatbot — Intent Recognition")
    setup_logging()
    ...
    if args.mode == "smoke":
        effective_batch = min(effective_batch, 4)
    batch_size, grad_accum = auto_batch_and_accum(
        args.gpu_mem_gb, args.batch_size or effective_batch, min_batch=2,
    )
    ...
    metrics = save_classification_report(
        ...
        labels=label_names, prefix="intent",
    )
