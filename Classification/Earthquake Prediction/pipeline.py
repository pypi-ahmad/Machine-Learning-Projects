...
def load_data():
    ...
    df = pd.read_csv("https://raw.githubusercontent.com/datasets/earthquake/main/data/earthquake.csv", sep=",")
...
def train_and_evaluate(X_train, X_test, y_train, y_test):
    ...
    try:
        ...
        test_ag = X_test.copy(); test_ag["magnitude"] = y_test.values
        with tempfile.TemporaryDirectory() as tmp:
            predictor = TabularPredictor(label="magnitude", path=tmp, verbosity=1)
            predictor.fit(train_ag, time_limit=180, presets="best_quality")
            results["AutoGluon"] = predictor.predict(test_ag.drop(columns=["magnitude"])).values
            try:
                probas["AutoGluon"] = predictor.predict_proba(test_ag.drop(columns=["magnitude"])).values
            except Exception:
                pass
