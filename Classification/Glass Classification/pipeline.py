...
def load_data():
    ...
    _fp = os.path.join(_data_dir, "glass.csv")
    if not os.path.exists(_fp):
        from kaggle.api.kaggle_api_extended import KaggleApi
        _api = KaggleApi(); _api.authenticate()
        _api.dataset_download_files("uciml/glass", path=_data_dir, unzip=True)
        _matches = _glob.glob(os.path.join(_data_dir, "**", "glass.csv"), recursive=True)
