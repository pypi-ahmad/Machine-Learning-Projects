#!/usr/bin/env python3
"""
Full pipeline for Customer segmentation for an E-commerce company
...
def main():
    ...
    cprint("The Top Five 'invoiceno' By Quantity :", 'blue')
    ...
    cprint("invoiceno Starts with C:",'blue')
    df['invoiceno'].str.startswith('C').value_counts()

    cprint("invoiceno Starts with A or C:",'blue')
    ...
    cprint("Number of Missing Values",'blue')
    ...
    cprint("The Number of Observations in the 'unitprice' Column Which Equal to 0",'blue')
    df[df['unitprice']==0][['invoiceno', 'stockcode', 'description', 'quantity', 'invoicedate', 'unitprice', 'customerid', 'country']].head()

    cprint("The Number of Observations in the 'unitprice' Column Which are Below 0",'blue')
    df[df['unitprice']<0][['invoiceno', 'stockcode', 'description', 'quantity', 'invoicedate', 'unitprice', 'customerid', 'country']].head()
    ...
    cprint("Customer Number by Orders (Bottom 5)",'blue')
    ...
    cprint("Customer Status According to the Number of Items in the Orders (Bottom 5)",'blue')
    ...
    cprint("Customer Status According to the Spendings (Bottom 5)",'blue')
    ...
    # This is our dataset which was performed some functions by first_looking() Def at the begining of study.
    ...
    # This is our raw datatset which we have at the begining of study.

    df0["InvoiceNo"].str.startswith('C').value_counts()

    df0["InvoiceNo"].str.startswith('C').value_counts(normalize=True)*100

    df[df["invoiceno"].str.startswith('C')]['customerid'].nunique() / df['customerid'].nunique()
    ...
    df.groupby("customerid")[["stockcode"]].nunique().sort_values(by="stockcode", ascending=False)
    ...
    df_uk.groupby("stockcode")["quantity"].sum().sort_values(ascending=False).head(1)
    ...
    cprint("Top 10 Demanded Products By Quantity",'blue')
    df_uk.groupby("stockcode")['quantity'].sum().sort_values(ascending=False)[:10].iplot(kind="bar", width=0.5, color='cyan');

    cprint("Top 10 Demanded Products By Total_Price",'blue')
    df_uk.groupby("stockcode")['total_price'].sum().sort_values(ascending=False)[:10].iplot(kind="bar", width=0.5, color='#46ADF5');
    ...
    customer_rfm[customer_rfm['Customer_RFM_Score']=='123'].head(3)
    ...
    # we will continue to the analyses with power tranformed data
...
if __name__ == "__main__":
    ...
    _parser = _ap.ArgumentParser(description="Full pipeline for Customer segmentation for an E-commerce company")
