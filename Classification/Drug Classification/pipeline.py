...
def load_data():
    ...
    _fp = os.path.join(_data_dir, "drug200.csv")
    if not os.path.exists(_fp):
        from kaggle.api.kaggle_api_extended import KaggleApi
        _api = KaggleApi(); _api.authenticate()
        _api.dataset_download_files("prathamtripathi/drug-classification", path=_data_dir, unzip=True)
        _matches = _glob.glob(os.path.join(_data_dir, "**", "drug200.csv"), recursive=True)
        if _matches: _fp = _matches[0]
        print(f"Downloaded prathamtripathi/drug-classification from Kaggle")
    df = pd.read_csv(_fp)
...
def train_and_evaluate(X_train, X_test, y_train, y_test):
    ...
    try:
        ...
        import tempfile
        t0 = time.perf_counter()
        train_ag = X_train.copy(); train_ag["Drug"] = y_train.values
        test_ag = X_test.copy(); test_ag["Drug"] = y_test.values
        with tempfile.TemporaryDirectory() as tmp:
            predictor = TabularPredictor(label="Drug", path=tmp, verbosity=0)
            predictor.fit(train_ag, time_limit=120, presets="medium_quality")
            results["AutoGluon"] = predictor.predict(test_ag.drop(columns=["Drug"])).values
            try:
                probas["AutoGluon"] = predictor.predict_proba(test_ag.drop(columns=["Drug"])).values
            except Exception:
                pass
