...
def load_data():
    ...
    _fp = os.path.join(_data_dir, "HR_comma_sep.csv")
    if not os.path.exists(_fp):
        from kaggle.api.kaggle_api_extended import KaggleApi
        _api = KaggleApi(); _api.authenticate()
        _api.dataset_download_files("giripujar/hr-analytics", path=_data_dir, unzip=True)
        _matches = _glob.glob(os.path.join(_data_dir, "**", "HR_comma_sep.csv"), recursive=True)
        if _matches: _fp = _matches[0]
        print(f"Downloaded giripujar/hr-analytics from Kaggle")
    df = pd.read_csv(_fp)
...
def train_and_evaluate(X_train, X_test, y_train, y_test):
    ...
    try:
        ...
        import tempfile
        t0 = time.perf_counter()
        train_ag = X_train.copy(); train_ag["left"] = y_train.values
        test_ag = X_test.copy(); test_ag["left"] = y_test.values
        with tempfile.TemporaryDirectory() as tmp:
            predictor = TabularPredictor(label="left", path=tmp, verbosity=0)
            predictor.fit(train_ag, time_limit=120, presets="medium_quality")
            results["AutoGluon"] = predictor.predict(test_ag.drop(columns=["left"])).values
            try:
                probas["AutoGluon"] = predictor.predict_proba(test_ag.drop(columns=["left"])).values
            except Exception:
                pass
