...
This project implements a **Classification** pipeline for **Fraud Detection Using ML**.
...
**Standardized data path:** `data/fraud_detection_using_ml/`
...
```mermaid
flowchart TD
    A[Load payment_fraud.csv] --> B[Label mapping]
...
Automated tests are available in `tests/test_p014_*.py`:

```bash
python -m pytest tests/test_p014_*.py -v
```
