#!/usr/bin/env python3
"""
Breast Cancer Wisconsin (PyCaret)
==================================
Classify breast tumours as malignant (M) or benign (B) using cell-nucleus features.
...
KAGGLE_SLUG = "uciml/breast-cancer-wisconsin-data"


def get_data(data_dir):
    ds_path = download_kaggle_dataset(KAGGLE_SLUG, data_dir, dataset_name="breast-cancer-wisconsin-data")
...
def preprocess(df):
    # Drop id and spurious unnamed columns
...
def main():
    args = parse_common_args("Breast Cancer Wisconsin — PyCaret Classification")
    setup_logging()
    set_seed(args.seed, deterministic=True)
    configure_cuda_allocator()
    paths = project_paths(__file__)
    logger.info("=== Breast Cancer Wisconsin (PyCaret) ===")
    ...
    try:
        ...
        dataset_missing_metrics(paths["outputs"], "Breast Cancer Wisconsin", ["https://www.kaggle.com/datasets/uciml/breast-cancer-wisconsin-data"])
        return
    ...
    if target not in df.columns:
        target = df.columns[-1]
        logger.warning("'diagnosis' column not found, using last column: %s", target)
