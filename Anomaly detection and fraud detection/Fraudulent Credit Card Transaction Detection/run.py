...
=======================================
Detect fraudulent credit-card transactions. The dataset is highly imbalanced
(~0.17 % fraud), so fix_imbalance=True is used in the PyCaret setup.
...
def get_data(data_dir):
    ds_path = download_kaggle_dataset(KAGGLE_SLUG, data_dir, dataset_name="creditcardfraud")
...
def main():
    args = parse_common_args("Credit Card Fraud Detection — PyCaret Classification")
    setup_logging()
    set_seed(args.seed, deterministic=True)
    configure_cuda_allocator()
    paths = project_paths(__file__)
    logger.info("=== Credit Card Fraud Detection (PyCaret) ===")
    ...
    try:
        ...
        dataset_missing_metrics(paths["outputs"], "Credit Card Fraud", ["https://www.kaggle.com/datasets/mlg-ulb/creditcardfraud"])
        return
