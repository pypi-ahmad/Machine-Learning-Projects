"""Export utilities for ID Card KYC Parser.

Supports:
- JSON: full extraction result with fields, confidence, template info.
- CSV: one row per card with KYC fields as columns.
...
Usage::
    ...
    exporter = IDCardExporter(IDCardConfig())
...
log = logging.getLogger("id_card_kyc.export")
...
class IDCardExporter:
    ...
    def write(
        ...
        """Record one ID card extraction."""
        row = self._to_row(result, report, source, card_detected)
        ...
        if self.cfg.export_json:
            self._json_records.append(
                self._to_json_record(result, report, source, card_detected)
            )
    ...
    def __enter__(self) -> IDCardExporter:
        return self
    ...
    def _to_row(
        ...
        return {
            ...
            "document_type": _fval("document_type"),
            "full_name": _fval("full_name"),
            "id_number": _fval("id_number"),
            "date_of_birth": _fval("date_of_birth"),
            "nationality": _fval("nationality"),
            "gender": _fval("gender"),
            "expiry_date": _fval("expiry_date"),
            "issue_date": _fval("issue_date"),
