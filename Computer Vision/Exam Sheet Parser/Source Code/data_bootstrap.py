...
Downloads and prepares a public document layout dataset for testing
and benchmarking the OCR + layout parsing pipeline.

Uses the FUNSD dataset from Hugging Face (form understanding in
noisy scanned documents) as a proxy for exam sheet evaluation.

Usage::

    from data_bootstrap import ensure_exam_sheet_dataset

    data_root = ensure_exam_sheet_dataset()              # idempotent
    data_root = ensure_exam_sheet_dataset(force=True)    # force re-download
"""
...
def ensure_exam_sheet_dataset(*, force: bool = False) -> Path:
    """Download and prepare the exam sheet evaluation dataset.
    ...
    -------
    Path
        The project data root (``data/exam_sheet_parser/``).
    """
...
def _collect_images(data_path: Path, processed_dir: Path) -> None:
    """Collect all document images into processed/images/."""
    ...
    log.info(
        "[%s] Collected %d document images into %s",
