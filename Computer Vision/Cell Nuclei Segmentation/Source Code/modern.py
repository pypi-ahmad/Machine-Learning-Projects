"""Modern v2 pipeline — Cell Nuclei Segmentation.

Replaces: Notebook-only U-Net segmentation
Uses:     YOLO26m-seg for instance segmentation (primary, trainable)
          Optional MedSAM comparison track (set MEDSAM_COMPARE=1)

Pipeline: YOLO26m-seg → nuclei instance masks
Compare:  MedSAM side-by-side evaluation (if installed + checkpoint)

Fine-tune: python train_segmentation.py --data nuclei_dataset.yaml
...
@register("cell_nuclei_segmentation")
class CellNucleiSegmentationModern(CVProject):
    project_type = "segmentation"
    description = "Cell nuclei instance segmentation via YOLO-seg"
    ...
    def load(self):
        # Primary: YOLO-seg (trainable — fine-tune on nuclei data for best results)
        from utils.yolo import load_yolo
        from models.registry import resolve
        weights, ver, fallback = resolve("cell_nuclei_segmentation", "seg")
        self._yolo = load_yolo(weights)
        self._backend = "yolo"
        print(f"  [cell_nuclei] YOLO-seg loaded ({weights})")
        if fallback:
            print("  [cell_nuclei] Fine-tune: python train_segmentation.py --data nuclei.yaml")
    ...
    def _find_checkpoint(name: str):
        candidates = [
            Path(__file__).resolve().parents[2] / "models" / "cell_nuclei_segmentation" / name,
    ...
    def visualize(self, input_data, output):
        if isinstance(output, dict) and "masks" in output:
            ...
            n = len(output["masks"])
            cv2.putText(vis, f"Nuclei: {n} [MedSAM]", (10, 25),
