...
and benchmarking the checkbox + OCR extraction pipeline.
...
def ensure_form_dataset(*, force: bool = False) -> Path:
    """Download and prepare the form dataset.
    ...
    -------
    Path
        The project data root (``data/form_ocr_checkbox_extractor/``).
    """
...
def _collect_images(data_path: Path, processed_dir: Path) -> None:
    """Collect all form images into processed/images/."""
    ...
    log.info(
        "[%s] Collected %d form images into %s",
