"""Evaluation script for Waste Sorting Detector.

Runs YOLO ``val()`` on the waste detection dataset and prints / logs
per-class metrics (mAP@50, mAP@50-95, precision, recall).

Usage::

    python evaluate.py                                   # default model + dataset
    python evaluate.py --model runs/waste_sorting/train/weights/best.pt
"""
...
log = logging.getLogger("waste_sorting.evaluate")
