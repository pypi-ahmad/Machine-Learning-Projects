"""Evaluation script for Document Layout Block Detector.

Runs YOLO ``val()`` on the layout detection dataset and prints
per-class metrics (mAP@50, mAP@50-95, precision, recall).

Usage::

    python evaluate.py
    python evaluate.py --model runs/doc_layout/train/weights/best.pt
"""
...
log = logging.getLogger("doc_layout.evaluate")


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Evaluate document layout detection model")
...
def main(argv: list[str] | None = None) -> None:
    ...
    # 3. Print summary
    box = metrics.box
    log.info("──── Layout Detection Results ────")
