"""Training script for Document Layout Block Detector.

Downloads the document layout dataset (via data_bootstrap) and delegates
training to the shared ``train/train_detection.py`` helper.

Usage::

    python train.py                          # defaults — 50 epochs, imgsz 1024
...
log = logging.getLogger("doc_layout.train")


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train document layout detection model")
...
def main(argv: list[str] | None = None) -> None:
    ...
    # 1. Ensure dataset
    data_root = ensure_layout_dataset(force=args.force_download)
    ...
    train_detection(
        ...
        name=args.name,
        registry_project="document_layout_block_detector",
    )
