...
def predict():
    ...
    for i in range(start[0], num[0]):
        ...
        # BUG FIX: label.sort() was destructively sorting the label list on every call,
        # corrupting the index-to-label mapping. Use sorted() on a copy instead.
        sorted_label = sorted(label)
        _true = sorted_label[top[2]]
        pa['image'] = f'{UPLOAD_FOLDER}/{i + 500}.jpg'
        x = dict()
        x[_true] = float("{:.2f}".format(pred[0][top[2]] * 100))
        x[sorted_label[top[1]]] = float("{:.2f}".format(pred[0][top[1]] * 100))
        x[sorted_label[top[0]]] = float("{:.2f}".format(pred[0][top[0]] * 100))
        pa['result'] = x
