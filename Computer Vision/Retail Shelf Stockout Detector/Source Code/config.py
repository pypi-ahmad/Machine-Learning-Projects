"""Retail Shelf Stockout Detector — configuration loader.

Loads zone definitions, thresholds, model settings, and export options
from a YAML or JSON config file.

Usage::

    from config import load_config, StockoutConfig

    cfg = load_config("zones.yaml")
    # or
    cfg = StockoutConfig()  # defaults
"""

from __future__ import annotations

import json
from dataclasses import dataclass, field
from pathlib import Path
from typing import Any


# ---------------------------------------------------------------------------
# Data classes
# ---------------------------------------------------------------------------
@dataclass
class ZoneConfig:
    """A single shelf-zone definition."""

    name: str
    polygon: list[tuple[int, int]]
    low_stock_threshold: int = 3
    classes: list[str] | None = None  # None = count all classes

    def to_dict(self) -> dict:
        return {
            "name": self.name,
            "polygon": self.polygon,
            "low_stock_threshold": self.low_stock_threshold,
            "classes": self.classes,
        }


@dataclass
class StockoutConfig:
    """Top-level project configuration."""

    # Model
    model: str = "yolo26m.pt"
    conf_threshold: float = 0.30
    iou_threshold: float = 0.45
    imgsz: int = 640
    device: str | None = None

    # Zones
    zones: list[ZoneConfig] = field(default_factory=list)
    default_low_stock_threshold: int = 3

    # Export
    export_dir: str = "outputs"
    save_events_csv: bool = True
    save_events_json: bool = True
    save_alert_snapshots: bool = True
    snapshot_cooldown_sec: float = 10.0

    # Inference
    show_display: bool = True
    save_video: bool = False
    output_fps: int = 25

    def to_dict(self) -> dict[str, Any]:
        d = {k: v for k, v in self.__dict__.items() if k != "zones"}
        d["zones"] = [z.to_dict() for z in self.zones]
        return d


# ---------------------------------------------------------------------------
# Loaders
# ---------------------------------------------------------------------------
def _parse_zones(raw: list[dict]) -> list[ZoneConfig]:
    """Parse zone dicts into ``ZoneConfig`` objects."""
    zones: list[ZoneConfig] = []
    for entry in raw:
        polygon = [tuple(pt) for pt in entry["polygon"]]
        zones.append(ZoneConfig(
            name=entry["name"],
            polygon=polygon,
            low_stock_threshold=entry.get("low_stock_threshold", 3),
            classes=entry.get("classes"),
        ))
    return zones


def load_config(path: str | Path) -> StockoutConfig:
    """Load a ``StockoutConfig`` from a YAML or JSON file.

    Parameters
    ----------
    path : str | Path
        Path to ``.yaml`` / ``.yml`` or ``.json`` config file.

    Returns
    -------
    StockoutConfig
    """
    path = Path(path)
    text = path.read_text(encoding="utf-8")

    if path.suffix in (".yaml", ".yml"):
        try:
            import yaml
            data = yaml.safe_load(text) or {}
        except ImportError:
            raise ImportError("PyYAML required to load YAML configs: pip install pyyaml")
    elif path.suffix == ".json":
        data = json.loads(text)
    else:
        raise ValueError(f"Unsupported config format: {path.suffix}")

    cfg = StockoutConfig()

    # Model settings
    cfg.model = data.get("model", cfg.model)
    cfg.conf_threshold = data.get("conf_threshold", cfg.conf_threshold)
    cfg.iou_threshold = data.get("iou_threshold", cfg.iou_threshold)
    cfg.imgsz = data.get("imgsz", cfg.imgsz)
    cfg.device = data.get("device", cfg.device)

    # Zones
    if "zones" in data:
        cfg.zones = _parse_zones(data["zones"])
    cfg.default_low_stock_threshold = data.get(
        "default_low_stock_threshold", cfg.default_low_stock_threshold,
    )

    # Export
    cfg.export_dir = data.get("export_dir", cfg.export_dir)
    cfg.save_events_csv = data.get("save_events_csv", cfg.save_events_csv)
    cfg.save_events_json = data.get("save_events_json", cfg.save_events_json)
    cfg.save_alert_snapshots = data.get("save_alert_snapshots", cfg.save_alert_snapshots)
    cfg.snapshot_cooldown_sec = data.get("snapshot_cooldown_sec", cfg.snapshot_cooldown_sec)

    # Inference
    cfg.show_display = data.get("show_display", cfg.show_display)
    cfg.save_video = data.get("save_video", cfg.save_video)
    cfg.output_fps = data.get("output_fps", cfg.output_fps)

    return cfg


def save_config(cfg: StockoutConfig, path: str | Path) -> None:
    """Persist a config to disk (JSON)."""
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(cfg.to_dict(), indent=2), encoding="utf-8")


def default_sample_config() -> StockoutConfig:
    """Return a demo config with two example zones for documentation."""
    cfg = StockoutConfig()
    cfg.zones = [
        ZoneConfig(
            name="Shelf-A",
            polygon=[(50, 100), (400, 100), (400, 350), (50, 350)],
            low_stock_threshold=3,
        ),
        ZoneConfig(
            name="Shelf-B",
            polygon=[(420, 100), (780, 100), (780, 350), (420, 350)],
            low_stock_threshold=5,
        ),
    ]
    return cfg
