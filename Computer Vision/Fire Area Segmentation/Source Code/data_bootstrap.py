"""Fire Area Segmentation -- idempotent dataset bootstrap."""
...
PROJECT_KEY = "fire_area_segmentation"
...
def ensure_fire_dataset(force: bool = False) -> Path:
    """Download and prepare the fire segmentation dataset (idempotent).
    """Download and prepare the fire segmentation dataset (idempotent).
...
def _collect_media(raw_dir: Path, media_dir: Path) -> None:
    """Copy fire images from raw download into processed/media/."""
...
if __name__ == "__main__":
    ...
    path = ensure_fire_dataset(force=force)
