"""Train Wildlife Image Classification — classification.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train Wildlife Image Classification")
    ...
    if args.data is None:
        data_path = DatasetResolver().resolve("wildlife_classification", force=args.force_download)
    ...
    train_classification(
        ...
        lr=args.lr,
        device=args.device,
        save_path=str(Path(__file__).parent / "runs" / "wildlife_cls" / "best_model.pt"),
        registry_project="wildlife_classification",
    )
