"""Document Type Classifier Router — training script.

Trains a classification model on 16 document types using the shared
training pipeline.
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Train Document Type Classifier"
    )
    ...
    save_dir = Path(__file__).parent / "runs" / "document_cls"
    ...
    stats = train_classification(
        ...
        registry_project="document_type_classifier_router",
    )
