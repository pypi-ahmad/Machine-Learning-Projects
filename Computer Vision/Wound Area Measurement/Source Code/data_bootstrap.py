"""Wound Area Measurement -- idempotent dataset bootstrap."""
...
PROJECT_KEY = "wound_area_measurement"
...
def ensure_wound_dataset(force: bool = False) -> Path:
    """Download and prepare the wound segmentation dataset (idempotent).
    """Download and prepare the wound segmentation dataset (idempotent).
...
def _collect_media(raw_dir: Path, media_dir: Path) -> None:
    """Copy wound images from raw download into processed/media/."""
...
if __name__ == "__main__":
    ...
    path = ensure_wound_dataset(force=force)
