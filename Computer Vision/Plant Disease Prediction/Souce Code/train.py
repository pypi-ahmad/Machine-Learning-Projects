"""Train Plant Disease Prediction — classification (38 classes).
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train Plant Disease Prediction (classification)")
    ...
    if args.data is None:
        data_path = DatasetResolver().resolve("plant_disease", force=args.force_download)
    ...
    train_classification(
        ...
        lr=args.lr,
        device=args.device,
        save_path=str(Path(__file__).parent / "runs" / "plant_disease_cls" / "best_model.pt"),
        registry_project="plant_disease",
    )
