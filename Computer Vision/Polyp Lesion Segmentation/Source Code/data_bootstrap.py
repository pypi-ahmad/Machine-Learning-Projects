"""Polyp Lesion Segmentation -- idempotent dataset bootstrap."""
...
PROJECT_KEY = "polyp_lesion_segmentation"
...
def ensure_polyp_dataset(force: bool = False) -> Path:
    """Download and prepare the polyp segmentation dataset (idempotent).
    """Download and prepare the polyp segmentation dataset (idempotent).
...
def _collect_media(raw_dir: Path, media_dir: Path) -> None:
    """Copy polyp images from raw download into processed/media/."""
...
if __name__ == "__main__":
    ...
    path = ensure_polyp_dataset(force=force)
