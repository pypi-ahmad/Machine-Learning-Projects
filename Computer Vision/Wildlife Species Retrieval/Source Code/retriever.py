"""Wildlife Species Retrieval — retrieval engine.

Combines the embedder and index to answer visual similarity queries.
"""

from __future__ import annotations

from dataclasses import dataclass
from typing import TYPE_CHECKING

import numpy as np

if TYPE_CHECKING:
    from config import WildlifeConfig

from embedder import WildlifeEmbedder
from index import SearchHit, WildlifeIndex


@dataclass
class RetrievalResult:
    """Full retrieval result for one query."""

    query_path: str | None
    query_embedding: np.ndarray
    hits: list[SearchHit]

    @property
    def top_path(self) -> str | None:
        return self.hits[0].path if self.hits else None

    @property
    def top_score(self) -> float:
        return self.hits[0].score if self.hits else 0.0

    @property
    def species_votes(self) -> dict[str, float]:
        """Aggregate similarity by species."""
        votes: dict[str, float] = {}
        for h in self.hits:
            sp = h.species or "unknown"
            votes[sp] = votes.get(sp, 0.0) + h.score
        return dict(sorted(votes.items(), key=lambda kv: kv[1], reverse=True))


class WildlifeRetriever:
    """Top-k visual similarity retrieval from a wildlife embedding index."""

    def __init__(self, cfg: WildlifeConfig | None = None) -> None:
        if cfg is None:
            from config import WildlifeConfig
            cfg = WildlifeConfig()
        self.cfg = cfg
        self._embedder = WildlifeEmbedder(cfg)
        self._index: WildlifeIndex | None = None

    # ── lifecycle ──────────────────────────────────────────

    def load(self, index_path: str | None = None) -> None:
        self._embedder.load()
        idx_path = index_path or self.cfg.index_path
        self._index = WildlifeIndex.load(idx_path)

    def close(self) -> None:
        self._embedder.close()
        self._index = None

    @property
    def index(self) -> WildlifeIndex | None:
        return self._index

    # ── query API ─────────────────────────────────────────

    def query_image(
        self,
        image_bgr: np.ndarray,
        top_k: int | None = None,
        source: str | None = None,
        exclude_self: bool = True,
    ) -> RetrievalResult:
        if self._index is None:
            raise RuntimeError("Call load() first.")

        embedding = self._embedder.embed(image_bgr)
        k = top_k or self.cfg.top_k
        hits = self._index.search(
            embedding, top_k=k,
            min_similarity=self.cfg.min_similarity,
            exclude_self=exclude_self,
            self_path=source,
        )

        return RetrievalResult(
            query_path=source,
            query_embedding=embedding,
            hits=hits,
        )
