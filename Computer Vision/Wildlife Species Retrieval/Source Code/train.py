"""Wildlife Species Retrieval — training script.

Trains a species classifier (for optional reranking) using the shared
training pipeline.
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Train Wildlife Species Classifier (for reranking)"
    )
    ...
    if args.data is None:
        data_path = DatasetResolver().resolve(
            "wildlife_species_retrieval", force=args.force_download
        )
    ...
    save_dir = Path(__file__).parent / "runs" / "wildlife_cls"
    ...
    stats = train_classification(
        ...
        registry_project="wildlife_species_retrieval",
    )
