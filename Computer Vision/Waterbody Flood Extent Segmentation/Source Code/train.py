"""Waterbody & Flood Extent Segmentation — training & evaluation.

Usage::

    # Fine-tune YOLO26m-seg on water body data
...
def _train(args: argparse.Namespace) -> None:
    ...
    if args.data is None:
        data_path = DatasetResolver().resolve(
            "waterbody_flood_extent_segmentation", force=args.force_download,
        )
    ...
    train_segmentation(
        ...
        name="waterflood_seg",
        registry_project="waterbody_flood_extent_segmentation",
    )


def _evaluate(args: argparse.Namespace) -> None:
    """Run segmentation on dataset images and report coverage stats."""
    import cv2
    ...
    data_root = ensure_flood_dataset(force=args.force_download)
    ...
    print(f"Evaluating water segmentation on {max_imgs} image(s) ...")
    ...
    for idx, img_path in enumerate(images):
        ...
        total_regions += cov.instance_count
        total_water_px += cov.water_area_px
        total_image_px += cov.total_image_px

        if (idx + 1) % 10 == 0 or idx == len(images) - 1:
            print(f"  [{idx + 1}/{max_imgs}] {img_path.name}: "
                  f"{cov.instance_count} regions, coverage={cov.coverage_ratio:.2%}")

    ctrl.close()

    avg_cov = total_water_px / total_image_px if total_image_px else 0
    print(f"\n{'=' * 50}")
    print(f"Images evaluated:     {max_imgs}")
    print(f"Total water regions:  {total_regions}")
    print(f"Total water area:     {total_water_px:,} px")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Waterbody & Flood Extent Segmentation — train / evaluate",
    )
