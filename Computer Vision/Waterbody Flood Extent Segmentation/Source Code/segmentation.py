...
class SegmentationResult:
    """Output of a single-image water segmentation pass."""

    instances: list[WaterInstance] = field(default_factory=list)
...
class WaterSegmenter:
    """YOLO26m-seg wrapper for water / flood region extraction."""
    ...
    def load(self) -> None:
        ...
        weights, ver, fallback = resolve(
            "waterbody_flood_extent_segmentation", "seg",
        )
        if fallback:
            print(
                f"[segmentation] Using pretrained fallback: {weights}  "
                "(fine-tune on water/flood data for best results)"
            )
        else:
    ...
    def segment(self, image: np.ndarray) -> SegmentationResult:
        """Extract water regions from *image* (BGR, uint8)."""
    ...
    def _extract(self, result, hw: tuple[int, int]) -> SegmentationResult:
        ...
        if result.masks is not None and len(result.masks):
            ...
            for i in range(len(masks_data)):
                ...
                instances.append(WaterInstance(
                    mask=binary,
