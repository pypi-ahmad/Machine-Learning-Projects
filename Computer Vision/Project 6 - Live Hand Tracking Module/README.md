# P6: Hand Tracking – Volume Control (YOLO-Pose)
...
Tracks hand positions using YOLO-Pose for gesture-based volume control, replacing legacy MediaPipe hand tracking.
...
```bash
python -m core.runner hand_tracking_v2 --source 0 --import-all
```
...
run_camera("hand_tracking_v2", source=0)
```

### Train

```bash
cd "Project 6 - Live Hand Tracking Module"
...
Resolves weights via `models.registry.resolve("hand_tracking", "pose")`, falls back to `yolo26n-pose.pt`.
...
Configuration: `configs/datasets/hand_tracking.yaml`
...
- **Visualize**: Calls `output[0].plot()`, highlights wrist keypoints (indices 9, 10) with magenta circles.
...
| Dataset not found | Run `python -m utils.data_downloader --project hand_tracking` |
