"""Train Traffic Sign Recognition — classification (43 classes, GTSRB).
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train Traffic Sign Recognition (classification)")
    ...
    if args.data is None:
        data_path = DatasetResolver().resolve("traffic_sign_recognition", force=args.force_download)
    ...
    train_classification(
        ...
        lr=args.lr,
        device=args.device,
        save_path=str(Path(__file__).parent / "runs" / "traffic_sign_cls" / "best_model.pt"),
        registry_project="traffic_sign_recognition",
    )
