"""Modern registry entry for Driver Drowsiness Monitor.

Registers the project with ``core/registry.py`` so it can be
discovered and launched via the unified CLI.

Pipeline: MediaPipe Face Mesh → EAR/MAR/head-pose → alert mgr.
"""
...
@register("driver_drowsiness_monitor")
class DriverDrowsinessModern(CVProject):
    """Real-time driver drowsiness detection via facial landmarks."""
    ...
    def load(self) -> None:
        ...
        self._pipeline = DrowsinessPipeline(self.cfg)
        self._pipeline.load()
        self._validator = DrowsinessValidator(self.cfg)
        self._loaded = True
    ...
    def predict(self, input_data, **kwargs):
        ...
        if isinstance(input_data, np.ndarray):
            ...
            return {
                ...
                "alerts": [a.alert_type for a in result.alerts],
            }
    ...
    def setup(self, **kwargs) -> None:
        ...
        self.cfg = (
            load_config(config_path) if config_path else DrowsinessConfig()
        )
        if kwargs.get("ear_threshold"):
            self.cfg.ear_threshold = kwargs["ear_threshold"]
        if kwargs.get("mar_threshold"):
            self.cfg.mar_threshold = kwargs["mar_threshold"]
