"""Train Skin Cancer Detection — classification (7 classes, HAM10000).
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train Skin Cancer Detection (classification)")
    ...
    if args.data is None:
        data_path = DatasetResolver().resolve("skin_cancer_detection", force=args.force_download)
    ...
    train_classification(
        ...
        lr=args.lr,
        device=args.device,
        save_path=str(Path(__file__).parent / "runs" / "skin_cancer_cls" / "best_model.pt"),
        registry_project="skin_cancer_detection",
    )
