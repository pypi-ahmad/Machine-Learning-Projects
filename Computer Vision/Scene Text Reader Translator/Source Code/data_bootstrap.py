...
Uses the COCO-Text subset from Hugging Face as the dataset source.
...
def ensure_scene_text_dataset(*, force: bool = False) -> Path:
    """Download and prepare the scene text dataset.
    ...
    -------
    Path
        The project data root (``data/scene_text_reader_translator/``).
    """
...
def _collect_images(data_path: Path, processed_dir: Path) -> None:
    """Collect all scene images into processed/images/."""
    ...
    log.info(
        "[%s] Collected %d scene text images into %s",
