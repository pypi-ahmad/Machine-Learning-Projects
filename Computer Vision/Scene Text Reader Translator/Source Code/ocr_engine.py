"""OCR engine wrapper for Scene Text Reader Translator.
...
Usage::
    ...
    engine = OCREngine(SceneTextConfig())
...
log = logging.getLogger("scene_text.ocr_engine")


@dataclass
class OCRBlock:
    """Single OCR text detection from a scene image."""
...
class OCREngine:
    """PaddleOCR-first wrapper with lazy initialisation for scene text."""
    ...
    @staticmethod
    def _from_paddle_result(result) -> list[OCRBlock]:
        blocks: list[OCRBlock] = []
        if not result:
            return blocks
        ...
        for item in lines:
            ...
            if isinstance(item, dict):
                ...
                continue
            if not text or bbox_raw is None:
                continue
            bbox = [[int(p[0]), int(p[1])] for p in bbox_raw]
            cx = int(np.mean([p[0] for p in bbox]))
            cy = int(np.mean([p[1] for p in bbox]))
