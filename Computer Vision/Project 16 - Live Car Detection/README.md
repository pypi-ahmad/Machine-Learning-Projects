...
Detects vehicles (cars, buses, trucks, motorcycles) in live video using YOLO26 with COCO class filtering.
...
```bash
python -m core.runner car_detection_v2 --source 0 --import-all
```
...
run_camera("car_detection_v2", source=0)
```

### Train

```bash
cd "Project 16 - Live Car Detection"
...
Resolves weights via `models.registry.resolve("car_detection", "detect")`, falls back to `yolo26n.pt`.
...
Configuration: `configs/datasets/car_detection.yaml`
...
- **Predict**: Runs YOLO detection filtered to vehicle COCO classes (car=2, bus=5, truck=7, motorcycle=3) with confidence 0.4.
...
| Dataset not found | Run `python -m utils.data_downloader --project car_detection` |
