"""Training script for Sports Ball Possession Tracker.

Downloads the sports detection dataset (via data_bootstrap) and delegates
...
log = logging.getLogger("sports_possession.train")


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train sports detection model")
...
def main(argv: list[str] | None = None) -> None:
    ...
    data_root = ensure_sports_dataset(force=args.force_download)
    ...
    train_detection(
        ...
        name=args.name,
        registry_project="sports_ball_possession_tracker",
    )
