# P21: Volume Controller (YOLO-Pose)
...
Controls system volume via hand gesture distance using YOLO-Pose wrist keypoint detection.
...
```bash
python -m core.runner volume_controller_v2 --source 0 --import-all
```
...
run_camera("volume_controller_v2", source=0)
```

### Train

```bash
cd "Project 21 - Volume Controller"
...
Resolves weights via `models.registry.resolve("volume_controller", "pose")`, falls back to `yolo26n-pose.pt`.
...
Configuration: `configs/datasets/volume_controller.yaml`
...
- **Visualize**: Calls `output[0].plot()`, highlights wrist keypoints (indices 9, 10) with green/red circles.
...
| Dataset not found | Run `python -m utils.data_downloader --project volume_controller` |
