# P12: Real-Time Object Detection (YOLO)
...
Performs real-time object detection on camera or video using YOLO26, replacing the legacy MobileNet-SSD Caffe model.
...
```bash
python -m core.runner object_detection_v2 --source 0 --import-all
```
...
run_camera("object_detection_v2", source=0)
```

### Train

```bash
cd "Project 12 - Real Time Object Detection"
...
Resolves weights via `models.registry.resolve("object_detection", "detect")`, falls back to `yolo26n.pt`.
...
Configuration: `configs/datasets/object_detection.yaml`

Download method: **http** (auto-download enabled)

```bash
python -m utils.data_downloader --project object_detection
```
...
- **Predict**: Runs YOLO detection on frame with confidence 0.25.
...
| Dataset not found | Run `python -m utils.data_downloader --project object_detection` |
