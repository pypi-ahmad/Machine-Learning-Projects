"""Traffic Violation Analyzer — YOLO training entry-point.

Downloads the traffic detection dataset (if needed) and delegates to the
...
from data_bootstrap import ensure_traffic_dataset  # noqa: E402
from train.train_detection import train_detection   # noqa: E402

log = logging.getLogger("traffic_violation.train")


def main() -> None:
    parser = argparse.ArgumentParser(description="Traffic Violation Analyzer — Training")
    ...
    data_root = ensure_traffic_dataset(force=args.force_download)
    ...
    train_detection(
        ...
        batch=args.batch,
        imgsz=args.imgsz,
        project=str(REPO_ROOT / "runs" / "traffic_violation_analyzer"),
        name="train",
    )
