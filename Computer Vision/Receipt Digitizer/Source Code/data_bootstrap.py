...
and benchmarking the extraction pipeline.
...
def ensure_receipt_dataset(*, force: bool = False) -> Path:
    """Download and prepare the receipt dataset.
    ...
    -------
    Path
        The project data root (``data/receipt_digitizer/``).
    """
...
def _collect_images(data_path: Path, processed_dir: Path) -> None:
    """Collect all receipt images into processed/images/."""
    ...
    log.info("[%s] Collected %d receipt images into %s", PROJECT_KEY, count, images_dir)
