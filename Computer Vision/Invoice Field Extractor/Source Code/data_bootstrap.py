...
Downloads and prepares a public invoice / receipt dataset for
testing and benchmarking the OCR extraction pipeline.
...
def ensure_invoice_dataset(*, force: bool = False) -> Path:
    """Download and prepare the invoice dataset.
    ...
    -------
    Path
        The project data root (``data/invoice_field_extractor/``).
    """
    ...
    # Copy images into processed/images for easy access
...
def _collect_images(data_path: Path, processed_dir: Path) -> None:
    """Collect all invoice images into processed/images/."""
    ...
    log.info("[%s] Collected %d invoice images into %s", PROJECT_KEY, count, images_dir)
