"""Yoga Pose Correction Coach -- CLI entry point."""
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Yoga Pose Correction Coach -- pose analysis with correction hints",
    )
    p.add_argument(
        "--source", default="0",
...
def main(argv: list[str] | None = None) -> None:
    ...
    from config import YogaConfig, load_config
    ...
    if args.force_download:
        ...
        ensure_yoga_dataset(force=True)

    cfg = load_config(args.config) if args.config else YogaConfig()
    if args.no_smoothing:
        cfg.enable_smoothing = False

    ctrl = YogaCoachController(cfg)
    ctrl.load()
...
def _process_frame(frame, ctrl, cfg, exporter, draw_overlay, cv2, args, out_dir, idx):
    ...
    if not args.no_display or args.save_annotated:
        vis = draw_overlay(
            ...
            show_pose_label=cfg.show_pose_label,
            show_confidence=cfg.show_confidence,
            show_corrections=cfg.show_corrections,
        )
        if not args.no_display:
            cv2.imshow("Yoga Pose Correction Coach", vis)
        if args.save_annotated:
