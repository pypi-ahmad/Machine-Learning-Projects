"""Train Celebrity Face Recognition — classification (105 classes).
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train Celebrity Face Recognition (classification)")
    ...
    if args.data is None:
        data_path = DatasetResolver().resolve("celebrity_face_recognition", force=args.force_download)
    ...
    train_classification(
        ...
        lr=args.lr,
        device=args.device,
        save_path=str(Path(__file__).parent / "runs" / "celebrity_face_cls" / "best_model.pt"),
        registry_project="celebrity_face_recognition",
    )
