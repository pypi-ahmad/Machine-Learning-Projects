...
testing and benchmarking the extraction pipeline.
...
def ensure_card_dataset(*, force: bool = False) -> Path:
    """Download and prepare the business card dataset.
    ...
    -------
    Path
        The project data root (``data/business_card_reader/``).
    """
...
def _collect_images(data_path: Path, processed_dir: Path) -> None:
    """Collect all card images into processed/images/."""
    ...
    log.info("[%s] Collected %d card images into %s", PROJECT_KEY, count, images_dir)
