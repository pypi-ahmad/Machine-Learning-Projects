"""Train Emotion Recognition — classification (7 emotions).
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train Emotion Recognition (classification)")
    ...
    parser.add_argument("--imgsz", type=int, default=48)
    ...
    if args.data is None:
        data_path = DatasetResolver().resolve("emotion_recognition", force=args.force_download)
    ...
    train_classification(
        ...
        lr=args.lr,
        device=args.device,
        save_path=str(Path(__file__).parent / "runs" / "emotion_cls" / "best_model.pt"),
        registry_project="emotion_recognition",
    )
