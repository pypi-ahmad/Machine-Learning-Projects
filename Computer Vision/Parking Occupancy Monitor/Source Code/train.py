"""Parking Occupancy Monitor — YOLO training entry-point.

Downloads the vehicle / parking-lot dataset (if needed) and delegates to
the shared ``train/train_detection.py`` pipeline.
...
log = logging.getLogger("parking_occupancy.train")


def main() -> None:
    parser = argparse.ArgumentParser(description="Parking Occupancy Monitor — Training")
    ...
    train_detection(
        ...
        batch=args.batch,
        imgsz=args.imgsz,
        project=str(REPO_ROOT / "runs" / "parking_occupancy_monitor"),
        name="train",
    )
