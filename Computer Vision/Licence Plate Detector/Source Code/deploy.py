...
def detectx (frame, model):
    ...
    results = model(frame)
    # Ultralytics YOLO returns list of Results; extract boxes as normalized xyxy + conf + cls
    r = results[0]
    boxes = r.boxes
    if boxes is not None and len(boxes):
        import torch as _t
        xyxyn = boxes.xyxyn  # normalized coordinates
        conf = boxes.conf.unsqueeze(1)
        cls = boxes.cls.unsqueeze(1)
        combined = _t.cat([xyxyn, conf, cls], dim=1)
        labels, cordinates = combined[:, -1], combined[:, :-1]
    else:
        import torch as _t
        labels = _t.tensor([])
        cordinates = _t.tensor([])
...
def main(img_path=None, vid_path=None,vid_out = None):
    ...
    model_path = paths.models("licence_plate_detector") / "best.pt"
