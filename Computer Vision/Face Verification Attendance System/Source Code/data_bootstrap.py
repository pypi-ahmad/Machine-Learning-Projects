"""Dataset bootstrap for Face Verification Attendance System.

Prepares a multi-identity public face dataset for enrollment and
verification evaluation. The bootstrap keeps an ImageFolder-style
layout so each identity has its own directory.
"""
...
log = logging.getLogger("face_attendance.data_bootstrap")

PROJECT_KEY = "face_verification_attendance"
...
def ensure_face_attendance_dataset(*, force: bool = False) -> Path:
    """Download and prepare the face attendance evaluation dataset."""
...
def _write_info(
    ...
    *,
    ...
    info = {
        ...
        "description": "Multi-identity face dataset prepared for enrollment and verification evaluation.",
        "identity_count": identity_count,
        "image_count": image_count,
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Prepare the face verification attendance dataset",
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    data_root = ensure_face_attendance_dataset(force=args.force)
    print(data_root)
