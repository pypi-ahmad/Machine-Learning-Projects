...
Downloads and prepares a public multi-identity face dataset (LFW)
from Hugging Face for testing the enrollment + verification pipeline.

Usage::

    from data_bootstrap import ensure_face_attendance_dataset

    data_root = ensure_face_attendance_dataset()           # idempotent
    data_root = ensure_face_attendance_dataset(force=True)  # force
"""
...
def ensure_face_attendance_dataset(*, force: bool = False) -> Path:
    ...
    -------
    Path
        The project data root (``data/face_verification_attendance/``).
    """
...
def _collect_images(data_path: Path, processed_dir: Path) -> None:
    """Collect face images into processed/images/."""
    ...
    log.info(
        "[%s] Collected %d face images into %s",
