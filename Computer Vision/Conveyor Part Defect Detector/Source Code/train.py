"""Conveyor Part Defect Detector — YOLO training entry-point.

Downloads the industrial defect dataset (if needed) and delegates to the
...
from data_bootstrap import ensure_defect_dataset    # noqa: E402
from train.train_detection import train_detection   # noqa: E402

log = logging.getLogger("conveyor_defect.train")


def main() -> None:
    parser = argparse.ArgumentParser(description="Conveyor Part Defect Detector — Training")
    ...
    train_detection(
        ...
        batch=args.batch,
        imgsz=args.imgsz,
        project=str(REPO_ROOT / "runs" / "conveyor_part_defect_detector"),
        name="train",
    )
