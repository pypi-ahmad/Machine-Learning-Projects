...
This project implements a **Classification** pipeline for **XgBoost ALgorithm**. The target variable is `Price`.
...
**Standardized data path:** `data/xgboost_algorithm/`
...
```mermaid
flowchart TD
    A[Load melb_data.csv] --> B[Train/test split]
...
Automated tests are available in `tests/test_p044_*.py`:

```bash
python -m pytest tests/test_p044_*.py -v
```
