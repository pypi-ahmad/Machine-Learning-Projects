...
def load_data():
    ...
    _fp = os.path.join(_data_dir, "Train.csv")
    if not os.path.exists(_fp):
        from kaggle.api.kaggle_api_extended import KaggleApi
        _api = KaggleApi(); _api.authenticate()
        _api.dataset_download_files("brijbhushannanda1979/bigmart-sales-data", path=_data_dir, unzip=True)
        _matches = _glob.glob(os.path.join(_data_dir, "**", "Train.csv"), recursive=True)
        if _matches: _fp = _matches[0]
        print(f"Downloaded brijbhushannanda1979/bigmart-sales-data from Kaggle")
    df = pd.read_csv(_fp)
...
def train_and_evaluate(X_train, X_test, y_train, y_test):
    ...
    try:
        ...
        m = CatBoostRegressor(iterations=300, learning_rate=0.05, depth=8, task_type="GPU",
    ...
    try:
        ...
        import tempfile
        t0 = time.perf_counter()
        train_ag = X_train.copy(); train_ag["Item_Outlet_Sales"] = y_train.values
        with tempfile.TemporaryDirectory() as tmp:
            predictor = TabularPredictor(label="Item_Outlet_Sales", path=tmp, problem_type="regression", verbosity=0)
    ...
    try:
        ...
        if X_train.shape[0] <= 10000 and X_train.shape[1] <= 500:
            t0 = time.perf_counter()
            m = TabPFNRegressor(device="cuda", n_estimators=32)
