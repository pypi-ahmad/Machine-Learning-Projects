"""
Auto-generated tests for: Regression/Flight Delay Prediction
...
DATA_PATH = os.path.join(os.path.dirname(__file__), "../../data/flight_delay_prediction/Jan_2020_ontime.csv")
...
class TestPreprocessing:
    ...
    def test_no_fully_null_columns(self):
        ...
        # Filter out unnamed/artifact columns before checking
    ...
    def test_fillna_does_not_crash(self):
        df = load_data(nrows=100)
        numeric_cols = df.select_dtypes(include=[np.number]).columns
        # Drop columns that are entirely null (median is NaN)
        non_null_cols = [c for c in numeric_cols if not df[c].isnull().all()]
        df[non_null_cols] = df[non_null_cols].fillna(df[non_null_cols].median())
        assert df[non_null_cols].isnull().sum().sum() == 0
