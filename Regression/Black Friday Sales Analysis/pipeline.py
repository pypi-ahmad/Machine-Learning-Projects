...
def load_data():
    ...
    if not os.path.exists(_fp):
        ...
        _api.dataset_download_files("sdolezel/black-friday", path=_data_dir, unzip=True)
        _matches = _glob.glob(os.path.join(_data_dir, "**", "train.csv"), recursive=True)
        if _matches: _fp = _matches[0]
        print(f"Downloaded sdolezel/black-friday from Kaggle")
    df = pd.read_csv(_fp)
...
def train_and_evaluate(X_train, X_test, y_train, y_test):
    ...
    try:
        ...
        import tempfile
        t0 = time.perf_counter()
        train_ag = X_train.copy(); train_ag["Purchase"] = y_train.values
        with tempfile.TemporaryDirectory() as tmp:
            predictor = TabularPredictor(label="Purchase", path=tmp, problem_type="regression", verbosity=0)
