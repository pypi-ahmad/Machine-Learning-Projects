...
def load_data():
    df = pd.read_csv("https://raw.githubusercontent.com/datasets/gdp/master/data/gdp.csv", sep=",")
...
def train_and_evaluate(X_train, X_test, y_train, y_test):
    ...
    try:
        ...
        import tempfile
        t0 = time.perf_counter()
        train_ag = X_train.copy(); train_ag["Value"] = y_train.values
        with tempfile.TemporaryDirectory() as tmp:
            predictor = TabularPredictor(label="Value", path=tmp, problem_type="regression", verbosity=0)
