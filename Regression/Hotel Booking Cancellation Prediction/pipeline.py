...
def load_data():
    ...
    df = _hf_load("Tirumala/hotel_booking_demand", split="train").to_pandas()
...
def train_and_evaluate(X_train, X_test, y_train, y_test):
    ...
    try:
        ...
        import tempfile
        t0 = time.perf_counter()
        train_ag = X_train.copy(); train_ag["is_canceled"] = y_train.values
        with tempfile.TemporaryDirectory() as tmp:
            predictor = TabularPredictor(label="is_canceled", path=tmp, problem_type="regression", verbosity=1)
