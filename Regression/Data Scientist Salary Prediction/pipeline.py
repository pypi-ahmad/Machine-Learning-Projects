...
def load_data():
    ...
    df = _hf_load("inductiva/ds-salaries", split="train").to_pandas()
...
def train_and_evaluate(X_train, X_test, y_train, y_test):
    ...
    try:
        ...
        import tempfile
        train_ag = X_train.copy(); train_ag["salary_in_usd"] = y_train.values
        with tempfile.TemporaryDirectory() as tmp:
            predictor = TabularPredictor(label="salary_in_usd", path=tmp, problem_type="regression", verbosity=1)
