...
Approach: If the dataset contains user–recipe ratings, train Neural
          Collaborative Filtering (NCF).  Otherwise fall back to a
          content-based PyCaret regression predicting recipe rating /
          popularity from metadata features.
Run     : python "Recommendation Systems/8  Recipe recommendation system/run.py"
"""
import sys
...
KAGGLE_SLUG = "prashantsingh001/recipes-dataset-64k-dishes"
SEED = 42
EMBED_DIM = 64
...
USER_COLS = ["user_id", "userid", "user", "author_id", "reviewer_id", "contributor_id"]
ITEM_COLS = [
    "recipe_id", "recipeid", "item_id", "itemid", "dish_id",
    "recipe_name", "name", "title",
]
RATING_COLS = [
    "rating", "score", "stars", "review_score", "avg_rating",
    "aggregated_rating", "aggregatedrating",
]
TARGET_COLS = [
    "rating", "aggregated_rating", "aggregatedrating", "avg_rating",
    "score", "stars", "total_time", "calories", "prep_time",
]
...
def _run_pycaret_fallback(df: pd.DataFrame, out_dir: Path, args) -> dict:
    """Content-based: predict recipe rating/score from metadata features."""
    ...
    if target_col is None:
        for c in df.select_dtypes(include=[np.number]).columns:
            if df[c].nunique() > 5:
                ...
                break
    if target_col is None:
        raise ValueError("No suitable numeric target column found for regression.")
    ...
    logger.info("Auto-ML fallback: %d rows, target=%s, %d features", len(data), target_col, len(num_cols))
...
def main() -> None:
    ...
    args = parse_common_args("Recipe Recommendation — NCF")
    ...
    if _has_user_item(df):
        ...
        try:
            ...
            ax.set_title("Recipe Recommendation — Actual vs Predicted")
            fig.tight_layout()
