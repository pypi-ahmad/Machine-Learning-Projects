...
def load_data():
    ...
    _fp = os.path.join(_data_dir, "shared_articles.csv")
    if not os.path.exists(_fp):
        from kaggle.api.kaggle_api_extended import KaggleApi
        _api = KaggleApi(); _api.authenticate()
        _api.dataset_download_files("gspmoreira/articles-sharing-reading-from-cit-deskdrop", path=_data_dir, unzip=True)
        _matches = _glob.glob(os.path.join(_data_dir, "**", "shared_articles.csv"), recursive=True)
        if _matches: _fp = _matches[0]
        print(f"Downloaded gspmoreira/articles-sharing-reading-from-cit-deskdrop from Kaggle")
    df = pd.read_csv(_fp)
