#!/usr/bin/env python3
"""Movie Recommendation — NCF (MovieLens)

Dataset : https://www.kaggle.com/datasets/ciroexe/movielens-latest-datasets
Approach: Neural Collaborative Filtering on the classic MovieLens ratings.csv.
...
KAGGLE_SLUG = "ciroexe/movielens-latest-datasets"
SEED = 42
EMBED_DIM = 64
...
def _load_data(data_dir: Path) -> pd.DataFrame:
    """Prefer ratings.csv if present, else pick the best match."""
    # Direct search for ratings.csv (MovieLens convention)
    ...
    # Fallback: score every CSV by column overlap
    ...
    ITEM_COLS = ["movieId", "movie_id", "movieid", "item_id", "itemid"]
...
def _prepare(df: pd.DataFrame):
    USER_CANDS = ["userId", "user_id", "userid", "user"]
    ITEM_CANDS = ["movieId", "movie_id", "movieid", "item_id", "itemid"]
    RATING_CANDS = ["rating", "score", "stars"]

    user_col = _find_col(df, USER_CANDS)
    item_col = _find_col(df, ITEM_CANDS)
    rating_col = _find_col(df, RATING_CANDS)
...
def main() -> None:
    ...
    args = parse_common_args("Movie Recommendation — NCF")
    ...
    try:
        ...
        ax.set_title("Movie Recommendation — Actual vs Predicted")
        fig.tight_layout()
