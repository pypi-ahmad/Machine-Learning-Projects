#!/usr/bin/env python3
"""Hotel Recommendation — NCF

Dataset : https://www.kaggle.com/datasets/keshavramaiah/hotel-recommendation
Approach: Neural Collaborative Filtering predicting user–hotel scores.
...
KAGGLE_SLUG = "keshavramaiah/hotel-recommendation"
SEED = 42
EMBED_DIM = 64
...
USER_COLS = [
    "user_id", "userid", "author", "reviewer_id", "reviewerid",
    "member_id", "user", "reviewer",
]
ITEM_COLS = [
    "hotel_id", "hotelid", "hotel_name", "hotel", "property_id",
    "item_id", "itemid", "offering_id", "accommodation_id",
]
RATING_COLS = [
    "rating", "ratings", "overall_rating", "overall", "score",
    "reviewer_score", "average_score", "stars", "review_score",
]
...
def _load_data(data_dir: Path) -> pd.DataFrame:
    ...
    for f in files:
        try:
            sample = pd.read_csv(f, nrows=5) if f.suffix == ".csv" else pd.read_parquet(f).head(5)
        except Exception:
            continue
        score = 0
    ...
    logger.info("Loading %s (match score %d/3)", best_file.name, best_score)
    if best_file.suffix == ".csv":
        df = pd.read_csv(best_file, nrows=SAMPLE_LIMIT * 2)
    else:
...
def _prepare(df: pd.DataFrame):
    ...
    if user_col is None or item_col is None:
        # Fallback: pick first two categorical / integer columns
    ...
    if rating_col is None:
        # Try numeric columns that look like scores
        for c in df.select_dtypes(include=[np.number]).columns:
            if df[c].nunique() <= 20 and df[c].min() >= 0:
                rating_col = c
                logger.info("Auto-detected rating-like column: %s", c)
                break
        if rating_col is None:
            logger.info("No rating column found — using implicit 1.0")
            df["_rating"] = 1.0
            rating_col = "_rating"
...
def main() -> None:
    ...
    args = parse_common_args("Hotel Recommendation — NCF")
    ...
    try:
        ...
        ax.set_title("Hotel Recommendation — Actual vs Predicted")
        fig.tight_layout()
