#!/usr/bin/env python3
"""Event Recommendation System — NCF

Dataset : https://www.kaggle.com/datasets/kilanisikiru/event-dataset
Approach: Neural Collaborative Filtering (NCF) predicting user–event
          interaction scores.  Evaluated with MAE / RMSE / R².
"""
import sys
...
KAGGLE_SLUG = "kilanisikiru/event-dataset"
SEED = 42
EMBED_DIM = 64
...
USER_COLS = ["user_id", "userid", "user", "member_id", "visitor_id", "attendee_id"]
ITEM_COLS = ["event_id", "eventid", "item_id", "itemid", "event", "item"]
RATING_COLS = ["rating", "score", "interest", "rsvp", "interaction", "count"]
...
def _load_data(data_dir: Path) -> pd.DataFrame:
    """Find and load a suitable CSV / parquet from *data_dir*."""
    files = sorted(data_dir.rglob("*.csv")) + sorted(data_dir.rglob("*.parquet"))
    if not files:
        raise FileNotFoundError(f"No CSV/parquet files found under {data_dir}")

    best, best_df = None, None
    for f in files:
        try:
            df = pd.read_csv(f, nrows=5) if f.suffix == ".csv" else pd.read_parquet(f).head(5)
        except Exception:
            continue
        u = _find_col(df, USER_COLS)
        i = _find_col(df, ITEM_COLS)
        if u and i:
            best = f
            break
    ...
    if best is None:
        best = files[0]
        logger.warning("No file with recognised user+item cols; falling back to %s", best.name)

    logger.info("Loading %s", best)
    if best.suffix == ".csv":
...
def _prepare(df: pd.DataFrame):
    ...
    if user_col is None or item_col is None:
        # Fall back to first two object/int columns
    ...
    if rating_col is None:
        # Synthesise implicit rating = 1
        logger.info("No explicit rating column found — using implicit rating = 1.0")
        df["_rating"] = 1.0
        rating_col = "_rating"
    ...
    if len(df) > SAMPLE_LIMIT:
        ...
        logger.info("Sampled %d rows", SAMPLE_LIMIT)
...
def main() -> None:
    ...
    args = parse_common_args("Event Recommendation — NCF")
    ...
    try:
        ...
        ax.set_title("Event Recommendation — Actual vs Predicted")
        fig.tight_layout()
