#!/usr/bin/env python3
"""Restaurant Recommendation — NCF

Dataset : https://www.kaggle.com/datasets/uciml/restaurant-data-with-consumer-ratings
Approach: Neural Collaborative Filtering predicting user–restaurant ratings.
...
KAGGLE_SLUG = "uciml/restaurant-data-with-consumer-ratings"
SEED = 42
EMBED_DIM = 64
...
USER_COLS = [
    "userID", "user_id", "userid", "consumer_id", "consumerid",
    "reviewer_id", "user",
]
ITEM_COLS = [
    "placeID", "place_id", "placeid", "restaurant_id", "restaurantid",
    "rest_id", "item_id", "itemid", "restaurant",
]
RATING_COLS = [
    "rating", "overall_rating", "food_rating", "service_rating",
    "score", "stars",
]
...
def _load_data(data_dir: Path) -> pd.DataFrame:
    """Scan all CSVs, pick the one that contains user + item + rating cols."""
    ...
    # Prefer a file with rating-like content
...
def main() -> None:
    ...
    args = parse_common_args("Restaurant Recommendation — NCF")
    ...
    try:
        ...
        ax.set_title("Restaurant Recommendation — Actual vs Predicted")
        fig.tight_layout()
