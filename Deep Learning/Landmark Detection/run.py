#!/usr/bin/env python3
"""
Project 4 -- Landmark Detection

Dataset : Google Landmarks Dataset
Model   : swin_tiny_patch4_window7_224.ms_in22k_ft_in1k (timm)
...
from shared.utils import (
    ...
    parse_common_args, DatasetUnavailable, load_profile, resolve_config,
    write_split_manifest, EarlyStopping, run_with_oom_backoff)
...
def get_data():
    ...
    # Google Landmarks dataset only has CSV metadata on Kaggle;
    # actual images need separate download from GCS URLs in the CSVs.
    imgs = list(DATA_DIR.rglob("*.jpg")) + list(DATA_DIR.rglob("*.png"))
    if not imgs:
        raise DatasetUnavailable(
            "dataset_missing",
            "Google Landmarks only ships CSV metadata on Kaggle; "
            "actual images must be downloaded from GCS.",
            url="https://github.com/cvdfoundation/google-landmark",
            slug=KAGGLE,
            suggested_fix="Download images from GCS URLs listed in the CSV files. "
                          "See https://github.com/cvdfoundation/google-landmark",
        )
    for child in sorted(DATA_DIR.iterdir()):
        if child.is_dir() and child.name not in ("__MACOSX", "__pycache__"):
            return child
    return DATA_DIR
