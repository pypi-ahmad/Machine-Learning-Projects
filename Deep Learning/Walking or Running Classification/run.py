#!/usr/bin/env python3
"""
Project 21 -- Human Action Recognition (Walk / Run)
...
def get_data():
    ...
    # Organize flat images into class subdirs using CSV label mapping
    organized = DATA_DIR / "organized"
    if organized.is_dir() and any(organized.rglob("*.jpg")):
        return organized
    import shutil
    har_root = DATA_DIR / "Human Action Recognition"
    if not har_root.is_dir():
        har_root = DATA_DIR
    for csv_name, split_name in [("Training_set.csv", "train"), ("Testing_set.csv", "test")]:
        csv_path = har_root / csv_name
        ...
        if "label" not in df.columns:
            continue
        img_dir = har_root / split_name
        for _, row in df.iterrows():
            src = img_dir / row["filename"]
            if src.exists():
                dest = organized / split_name / str(row["label"])
