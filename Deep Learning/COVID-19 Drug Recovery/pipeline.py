...
def load_data():
    ...
    _fp = os.path.join(_data_dir, "Covid Data.csv")
    if not os.path.exists(_fp):
        from kaggle.api.kaggle_api_extended import KaggleApi
        _api = KaggleApi(); _api.authenticate()
        _api.dataset_download_files("meirnizri/covid19-dataset", path=_data_dir, unzip=True)
        _matches = _glob.glob(os.path.join(_data_dir, "**", "Covid Data.csv"), recursive=True)
        if _matches: _fp = _matches[0]
        print(f"Downloaded meirnizri/covid19-dataset from Kaggle")
    df = pd.read_csv(_fp)
...
def train_and_evaluate(X_train, X_test, y_train, y_test):
    ...
    try:
        ...
        import tempfile
        t0 = time.perf_counter()
        train_ag = X_train.copy(); train_ag["PATIENT_TYPE"] = y_train.values
        test_ag = X_test.copy(); test_ag["PATIENT_TYPE"] = y_test.values
        with tempfile.TemporaryDirectory() as tmp:
            predictor = TabularPredictor(label="PATIENT_TYPE", path=tmp, verbosity=0)
            predictor.fit(train_ag, time_limit=120, presets="medium_quality")
            results["AutoGluon"] = predictor.predict(test_ag.drop(columns=["PATIENT_TYPE"])).values
            try:
                probas["AutoGluon"] = predictor.predict_proba(test_ag.drop(columns=["PATIENT_TYPE"])).values
            except Exception:
                pass
