#!/usr/bin/env python
"""Build and execute FashionMNIST Autoencoder notebook"""
...
def install_if_missing(p, i=None):
    i = i or p
    try: __import__(i)
...
SEED, BATCH_SIZE, LEARNING_RATE, NUM_EPOCHS = 42, 128, 1e-3, 5
BOTTLENECK_DIM = 32
...
print(f"Device: {DEVICE}")""",
    # Dataset
    f"""print("Downloading FashionMNIST from https://github.com/zalandoresearch/fashion-mnist ...")
...
train_subset, val_subset = torch.utils.data.random_split(train_dataset, [train_size, val_size], generator=torch.Generator().manual_seed(SEED))
...
print(f"Split: {{len(train_subset)}} train, {{len(val_subset)}} val, {{len(test_dataset)}} test")""",
    ...
    def __init__(self, bottleneck_dim=32):
        super().__init__()
        self.enc_conv1 = nn.Sequential(nn.Conv2d(1, 16, 3, 1, 1), nn.ReLU(), nn.MaxPool2d(2, 2))
        self.enc_conv2 = nn.Sequential(nn.Conv2d(16, 32, 3, 1, 1), nn.ReLU(), nn.MaxPool2d(2, 2))
        self.fc_encode = nn.Linear(32 * 7 * 7, bottleneck_dim)
        self.fc_decode = nn.Linear(bottleneck_dim, 32 * 7 * 7)
        self.dec_conv1 = nn.Sequential(nn.ConvTranspose2d(32, 16, 3, 2, 1, 1), nn.ReLU())
        self.dec_conv2 = nn.Sequential(nn.ConvTranspose2d(16, 1, 3, 2, 1, 1), nn.Sigmoid())
    def encode(self, x):
        h = self.enc_conv1(x)
        h = self.enc_conv2(h)
...
for e in range(NUM_EPOCHS):
    ...
    print(f"  Epoch {e+1}: train={tl:.6f} val={vl:.6f}")
model.load_state_dict(torch.load(str(best_model_path)))
print(f"Best val: {best_val_loss:.6f}")""",
    # Metrics
    """def compute_metrics(m, l, d):
    m.eval()
    mse_data = []
    with torch.no_grad():
        for x, _ in l:
            x = x.to(d)
            r, _ = m(x)
            mse_data.extend(torch.mean((r - x) ** 2, dim=(1, 2, 3)).cpu().numpy())
...
# Write notebook
notebook_path = Path("Fashion MNIST Autoencoder.ipynb")
