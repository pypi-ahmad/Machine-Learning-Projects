...
def load_data():
    ...
    _d = fetch_openml(data_id=4353, as_frame=True, parser="auto")
    df = _d.frame
...
def train_and_evaluate(X_train, X_test, y_train, y_test):
    ...
    try:
        ...
        import tempfile
        t0 = time.perf_counter()
        train_ag = X_train.copy(); train_ag["csMPa"] = y_train.values
        with tempfile.TemporaryDirectory() as tmp:
            predictor = TabularPredictor(label="csMPa", path=tmp, problem_type="regression", verbosity=0)
