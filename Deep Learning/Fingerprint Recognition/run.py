#!/usr/bin/env python3
"""
Project 33 -- Fingerprint Recognition

Dataset : SOCOFing Fingerprint Dataset
Model   : efficientnet_b0.ra_in1k (timm)
...
_FINGER_RE = re.compile(
    r'^\d+__[MF]_(Left|Right)_(index|little|middle|ring|thumb)_finger_',
    re.IGNORECASE,
)
...
def get_data():
    ...
    organised = DATA_DIR / "organised"
    if organised.is_dir() and any(organised.iterdir()):
        return organised
    if not list(DATA_DIR.rglob("*.BMP")) and not list(DATA_DIR.rglob("*.bmp")):
        kaggle_download(KAGGLE, DATA_DIR)
    # Find flat directory with BMP images
    src = None
    for bmp in DATA_DIR.rglob("*.BMP"):
        src = bmp.parent; break
    if src is None:
        for bmp in DATA_DIR.rglob("*.bmp"):
            src = bmp.parent; break
    if src is None:
        return DATA_DIR
    # Reorganize into ImageFolder by finger type
    import shutil
    organised.mkdir(parents=True, exist_ok=True)
    for img in src.iterdir():
        m = _FINGER_RE.match(img.name)
        if m is None:
            continue
        cls = f"{m.group(1)}_{m.group(2)}"
        dest = organised / cls
