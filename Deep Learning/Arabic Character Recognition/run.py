#!/usr/bin/env python3
"""
Project 19 -- Arabic Handwritten Character Recognition
...
def get_data():
    ...
    # Organize flat PNGs (id_X_label_Y.png) into class subdirs
    organized = DATA_DIR / "organized"
    if organized.is_dir() and any(organized.rglob("*.png")):
        return organized
    import shutil
    for split_name in ["train", "test"]:
        src_dirs = [d for d in DATA_DIR.rglob(split_name) if d.is_dir()]
        if not src_dirs:
            continue
        src = src_dirs[0]
        for img in src.glob("*.png"):
            ...
            dest = organized / split_name / f"class_{label}"
